...
import {
  ...
  resolveGovernmentSignedPdfAbsUrl,
  type ContractDocumentDetailPayload,
  type ContractPublicValueInput,
  type GovernmentSignatureSendSessionAttachmentPublic,
...
export default function GovernmentSignDocumentPage() {
  ...
  const paramsInvalid = !signToken || !documentInstanceId

  const publicSignedPdfHref = useMemo(
    () => (paramsInvalid ? '' : resolveGovernmentSignedPdfAbsUrl(signToken, documentInstanceId)),
    [paramsInvalid, signToken, documentInstanceId],
  )
  ...
  const [attachmentModal, setAttachmentModal] = useState<GovernmentSignatureSendSessionAttachmentPublic | null>(null)
  ...
  const reloadDetail = useCallback(
    ...
    > => {
      const d = await fetchContractPublicDocumentDetail(signToken, documentInstanceId)
      if (isCancelled?.()) {
        return undefined
      }
      setDetail(d)
  ...
  }, [signToken, documentInstanceId, paramsInvalid, reloadDetail])
  ...
  const onSaveValues = async () => {
    ...
    setSaving(true)
    try {
      await postContractPublicDocumentValues(signToken, documentInstanceId, buildCustomerValuePayload())
  ...
  const onSaveConfirmationOnlyContent = async () => {
    ...
    setSaving(true)
    try {
      await postContractPublicDocumentValues(signToken, documentInstanceId, [], {
  ...
  const onComplete = async () => {
    ...
    setSaving(true)
    try {
      const data = await postContractPublicDocumentComplete(signToken, documentInstanceId, {
  ...
  const persistConfirmationOnlyChecks = useCallback(
    ...
    [detail, signToken, documentInstanceId, reloadDetail, scrollToNextPublicStep],
  )

  const inputRenderedPdfSrc = !paramsInvalid ? resolveContractRenderedPdfAbsUrl(signToken, documentInstanceId, 'input') : ''
  const finalRenderedPdfSrc = !paramsInvalid ? resolveContractRenderedPdfAbsUrl(signToken, documentInstanceId, 'final') : ''
  ...
  if (paramsInvalid) {
    body = (
      <div className="contract-public-sign-page__panel-danger-soft">
        ...
        <Link
          ...
          to={signToken ? `/government/sign/${encodeURIComponent(signToken)}` : '/'}
        >
          목록으로
        </Link>
      </div>
    ...
    body = (
      <div className="contract-public-sign-page__panel-danger-soft">
        ...
        <Link className="contract-public-sign-page__link contract-public-sign-page__link--after-block" to={`/government/sign/${encodeURIComponent(signToken)}`}>
          목록으로
        </Link>
      </div>
    )
      ...
      if (detail.document.status === 'completed') {
        ...
        body = (
          <div className="contract-public-sign-page__stack">
            ...
            <Link
              className="contract-public-sign-page__link contract-public-sign-page__link--after-block"
              to={`/government/sign/${encodeURIComponent(signToken)}`}
            >
              ← 문서 목록
            </Link>
          </div>
        ...
        <div className="contract-public-sign-page__stack">
          ...
          <GovernmentSignatureAttachmentReviewModal
          ...
          <SignatureModal
            ...
            onSave={async (blob) => {
              ...
              await postContractPublicDocumentSign(signToken, documentInstanceId, {
