...
export type GovernmentSignatureAttachmentReviewModalProps = {
  open: boolean
  onClose: () => void
  signToken: string
  attachment: GovernmentSignatureSendSessionAttachmentPublic | null
...
export function GovernmentSignatureAttachmentReviewModal({
  open,
  onClose,
  signToken,
  attachment,
  ...
  onConfirmed,
  onActionError,
}: GovernmentSignatureAttachmentReviewModalProps) {
  ...
  useEffect(() => {
    ...
    const url = resolveContractAttachmentViewAbsUrl(signToken, attachment.id)
    setImgError(null)
    setImgLoading(true)
    setImgUrl(null)
    ;(async () => {
  ...
  }, [open, attachment, signToken, loadNonce])
  ...
  const viewUrl = resolveContractAttachmentViewAbsUrl(signToken, attachment.id)
  const pdf = isPdfAttachment(attachment.mimeType, attachment.displayFilename)
      ...
      <>
        ...
        <FormButton
          ...
          onClick={() => {
            ...
            ;(async () => {
              try {
                const data = await postContractPublicAttachmentConfirm(signToken, attachment.id)
                onConfirmed({
