...
export async function fetchContractPublicSession(signToken: string): Promise<ContractPublicSessionPayload> {
  return publicRequest<ContractPublicSessionPayload>(`/api/government-support/public/signatures/${lc(signToken)}`)
}

export async function postContractPublicOpen(signToken: string): Promise<{ opened: boolean; metaRecorded?: boolean }> {
  return publicRequest(`/api/government-support/public/signatures/${lc(signToken)}/open`, {
    method: 'POST',
    body: JSON.stringify({}),
  })
}
...
export async function fetchContractPublicDocuments(signToken: string): Promise<{ documents: ContractDocumentRow[] }> {
  return publicRequest(`/api/government-support/public/signatures/${lc(signToken)}/documents`)
}
...
export async function postContractOtpVerify(signToken: string, code: string): Promise<{ verified?: boolean; status?: string }> {
  return publicRequest(`/api/government-support/public/signatures/${lc(signToken)}/otp/verify`, {
    method: 'POST',
    body: JSON.stringify({ code: String(code ?? '').replace(/\D/g, '').slice(0, 6) }),
  })
}

export async function fetchContractOtpStatus(signToken: string): Promise<{
  verified: boolean
  sendSessionStatus: string
  maskedPhone: string | null
}> {
  return publicRequest(`/api/government-support/public/signatures/${lc(signToken)}/otp/status`)
}
...
export type ContractDocumentDetailPayload = {
  ...
  completed?: boolean
  /** confirmation_only: gov_signature_evidences 행 존재 시 true */
  ...
  sendSessionAttachments?: GovernmentSignatureSendSessionAttachmentPublic[]
}

export type GovernmentSignatureSendSessionAttachmentPublic = {
  id: string
...
export function resolveContractAttachmentViewAbsUrl(signToken: string, attachmentId: string): string {
  return resolveApiUrl(`/api/government-support/public/signatures/${lc(signToken)}/attachments/${lc(attachmentId)}/view`)
}
...
export async function postContractPublicAttachmentConfirm(
  ...
  return publicRequest(`/api/government-support/public/signatures/${lc(signToken)}/attachments/${lc(attachmentId)}/confirm`, {
    method: 'POST',
    body: JSON.stringify({}),
  })
}
...
export async function fetchContractPublicDocumentDetail(
  ...
  const data = await publicRequest<ContractDocumentDetailPayload>(
    `/api/government-support/public/signatures/${lc(signToken)}/documents/${lc(documentInstanceId)}`,
  )
...
export function resolveContractPdfPreviewAbsUrl(signToken: string, documentInstanceId: string): string {
  return resolveApiUrl(`/api/government-support/public/signatures/${lc(signToken)}/documents/${lc(documentInstanceId)}/pdf`)
}
...
export function resolveContractRenderedPdfAbsUrl(
  ...
  const base = resolveApiUrl(`/api/government-support/public/signatures/${lc(signToken)}/documents/${lc(documentInstanceId)}/rendered-pdf`)
  const q = mode === 'input' ? '?mode=input' : '?mode=final'
  return `${base}${q}`
}

export function resolveGovernmentSignedPdfAbsUrl(signToken: string, documentInstanceId: string): string {
  return resolveApiUrl(`/api/government-support/public/signatures/${lc(signToken)}/documents/${lc(documentInstanceId)}/signed-pdf`)
}
...
export async function postContractPublicDocumentValues(
  ...
  return publicRequest(`/api/government-support/public/signatures/${lc(signToken)}/documents/${lc(documentInstanceId)}/values`, {
    method: 'POST',
...
export async function postContractPublicDocumentSign(
  ...
  return publicRequest(`/api/government-support/public/signatures/${lc(signToken)}/documents/${lc(documentInstanceId)}/sign`, {
    method: 'POST',
    body: JSON.stringify(body),
  })
}
...
export async function postContractPublicDocumentComplete(
  ...
  return publicRequest(`/api/government-support/public/signatures/${lc(signToken)}/documents/${lc(documentInstanceId)}/complete`, {
    method: 'POST',
    body: JSON.stringify(body),
  })
}
