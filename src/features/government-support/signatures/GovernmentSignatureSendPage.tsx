/**
 * 전자서명 발송 — USER / GA_STAFF. 관리자 템플릿은 /government/signature-templates 에서만 관리.
 */
...
import {
  ...
  getGovSignatureProfileSearchValidationMessage,
  searchCustomersForGovernmentSignatureSend,
  CONTRACT_SEND_CONFIRMATION_MAX_ITEMS,
  CONTRACT_SEND_CONFIRMATION_MAX_LABEL_LEN,
  CONTRACT_SEND_ATTACHMENTS_MAX,
  uploadUserGovernmentSignatureSendAttachment,
  type GovSignatureProfileSearchHit,
 ...
 */
function governmentSignatureSendSessionDetailMatchesPick(
  detail: SendSessionDetail | null,
  profileId: number | undefined,
  templateId: string | null,
): boolean {
  if (!detail || profileId == null || templateId == null || templateId === '') {
    return false
  }
...
function sniffGovernmentSignatureSendAttachmentMime(file: File): string {
...
function validateGovernmentSignatureSendAttachmentFile(file: File): { ok: true; mime: string } | { ok: false; message: string } {
  if (file.size > ATTACHMENT_MAX_BYTES) {
    return { ok: false, message: '첨부파일은 파일당 최대 20MB까지 업로드할 수 있습니다.' }
  }
  const mime = sniffGovernmentSignatureSendAttachmentMime(file)
...
export default function GovernmentSignatureSendPage() {
  ...
  const [customerHits, setCustomerHits] = useState<GovSignatureProfileSearchHit[]>([])
  ...
  const [selectedCustomer, setSelectedCustomer] = useState<GovSignatureProfileSearchHit | null>(null)
  ...
  const [confirmationTemplateFields, setConfirmationTemplateFields] = useState<GovSignatureConfirmationFieldRow[]>([])
  ...
  const executeCustomerSearch = useCallback(async () => {
    ...
    const validationMsg = getGovSignatureProfileSearchValidationMessage(effectiveQuery)
    if (validationMsg) {
    ...
    try {
      const hits = await searchCustomersForGovernmentSignatureSend(t, trimmed)
  ...
  useEffect(() => {
    ...
    setConfirmationFieldsError(null)
    void listUserGovernmentSignatureTemplateConfirmationFields(t, selectedTemplateId)
      .then((rows) => {
        if (cancelled) {
          return
        }
  ...
  const senderPrefillSatisfied = (tpl: UserGovernmentSignatureTemplateItem | null | undefined): boolean => {
  ...
  const scopedSendSessionDetail = useMemo((): SendSessionDetail | null => {
    if (!governmentSignatureSendSessionDetailMatchesPick(sessionDetail, selectedCustomer?.id, selectedTemplateId)) {
      return null
    }
  ...
  const scopedLastCreated = useMemo((): CreateSendSessionResult | null => {
    ...
    if (lastCreated.profileId !== scopedSendSessionDetail.profileId) {
      return null
    }
    return lastCreated
  ...
  const processPickedAttachmentFiles = async (files: File[]) => {
    ...
    for (const file of files) {
      const checked = validateGovernmentSignatureSendAttachmentFile(file)
      if (!checked.ok) {
        ...
        setAttachmentDrafts((prev) => {
          ...
          return [
            ...prev,
            {
              ...
              mimeType: sniffGovernmentSignatureSendAttachmentMime(file) || file.type || 'application/octet-stream',
              sizeBytes: file.size,
              required: true,
              uploadStatus: 'error',
      ...
      try {
        const up = await uploadUserGovernmentSignatureSendAttachment(t, selectedCustomer.id, file)
        setAttachmentDrafts((prev) =>
          prev.map((x) =>
            x.key === key
              ? {
