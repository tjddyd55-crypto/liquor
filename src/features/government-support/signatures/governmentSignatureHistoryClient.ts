...
export async function listUserSendSessions(
  ...
  const body = await apiRequest<{
    ...
    limit?: number
    offset?: number
  }>(`/api/government-support/signatures${suffix}`, { method: 'GET', token })
...
export async function cancelUserSendSession(token: string, sendSessionId: string): Promise<CancelUserSendSessionResult> {
  const body = await apiRequest<CancelUserSendSessionResult>(`/api/government-support/signatures/${encodeURIComponent(sendSessionId)}/cancel`, {
    method: 'PATCH',
    token,
    body: JSON.stringify({}),
  })
 ...
 */
export function buildGovSignaturePublicSignUrl(signToken: string): string {
  const raw = String(signToken ?? '').trim()
  const path = `/government/sign/${encodeURIComponent(raw)}`
