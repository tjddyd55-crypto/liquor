...
export type GovernmentSignatureSenderFieldDef = {
  fieldKey: string
  label: string
  required: boolean
  fieldType: string
...
export type GovSignatureConfirmationFieldRow = {
  id: string
  fieldKey: string
  label: string
...
function coerceUserConfirmationInputType(raw: unknown): GovSignatureConfirmationFieldRow['inputType'] {
...
function coerceUserConfirmationInputRole(raw: unknown): GovSignatureConfirmationFieldRow['inputRole'] {
  const role = String(raw ?? 'sender').trim()
  return role === 'customer' ? 'customer' : 'sender'
}

function coerceUserConfirmationField(raw: Record<string, unknown>): GovSignatureConfirmationFieldRow {
  return {
    id: String(raw.id ?? ''),
...
export type UserGovernmentSignatureTemplateItem = {
  id: string
  title: string
  ...
  templateMode: GovernmentSignatureTemplateMode
  pdfTemplateId: number | null
  pdfEngineTitle: string | null
  pdfFieldCount: number
  signatureFieldCount: number
  /** 설계사가 발송 전에 채워야 하는 PDF 필드(서버가 연결된 PDF 기준으로 계산) */
  senderFieldsForSend: GovernmentSignatureSenderFieldDef[]
  sendable: boolean
}

export type GovSignatureProfileSearchHit = {
  id: number
  name: string
...
export function getGovSignatureProfileSearchValidationMessage(raw: string): string | null {
  const q = raw.trim()
  if (!q) {
...
export async function uploadUserGovernmentSignatureSendAttachment(
  token: string,
  profileId: number,
  file: File,
): Promise<{
  ...
  const resolvedUrl = resolveApiUrl('/api/government-support/signatures/attachment-upload')
...
export async function listUserGovernmentSignatureTemplates(
  token: string,
): Promise<UserGovernmentSignatureTemplateItem[]> {
  const body = await apiRequest<{ templates?: UserGovernmentSignatureTemplateItem[] }>(
    '/api/government-support/signatures/send/templates',
    { method: 'GET', token },
  )
  const raw = body as { templates?: UserGovernmentSignatureTemplateItem[] }
 ...
 */
export async function listUserGovernmentSignatureTemplateConfirmationFields(
  token: string,
  templateId: string,
): Promise<GovSignatureConfirmationFieldRow[]> {
  const body = await apiRequest<{ fields?: unknown[] }>(
    `/api/government-support/signature-templates/${encodeURIComponent(templateId)}/confirmation-fields`,
    { method: 'GET', token },
  )
...
function dedupeGovernmentSignatureSendHitsById(
  rows: GovSignatureProfileSearchHit[],
): GovSignatureProfileSearchHit[] {
  const seen = new Set<number>()
  const out: GovSignatureProfileSearchHit[] = []
...
export async function searchCustomersForGovernmentSignatureSend(
  token: string,
  q: string,
): Promise<GovSignatureProfileSearchHit[]> {
  ...
  qs.set('q', trimmed)
  const body = await apiRequest<{ customers?: GovSignatureProfileSearchHit[] }>(
    `/api/government-support/signatures/profiles/search?${qs.toString()}`,
    { method: 'GET', token },
  )
  const raw = body as { customers?: GovSignatureProfileSearchHit[] }
  if (!raw?.customers || !Array.isArray(raw.customers)) {
    throw new ApiError('고객 검색 응답 형식이 올바르지 않습니다.', 500)
  }
  return dedupeGovernmentSignatureSendHitsById(raw.customers)
}

export async function createUserGovernmentSignatureSendSession(
  token: string,
  params: {
    profileId: number
    templateIds: string[]
