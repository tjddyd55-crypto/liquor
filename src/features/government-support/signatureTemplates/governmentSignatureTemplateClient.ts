...
export type GovernmentSignatureTemplateListItem = {
  id: string
  title: string
  ...
  createdAt: string
  updatedAt: string
  /** gov_signature_document_instances 건수(발송 이력 판단) */
  documentInstanceCount: number
  /** gov_signature_package_items 건수 */
...
export type GovernmentSignatureTemplateDetail = {
  id: string
  title: string
  description: string | null
  category: string | null
  ...
  pageCount: number | null
  ownerUserId: number | null
  governmentSignatureTemplateFieldsCount: number
...
export type GovernmentSignatureSendConfirmationItem = {
  id: string
  label: string
  required: boolean
  sortOrder: number
...
export async function listGovernmentSignatureTemplates(
  ...
  return raw.templates.map((t) => ({
    ...
    documentInstanceCount: Number((t as GovernmentSignatureTemplateListItem).documentInstanceCount ?? 0),
    packageItemCount: Number((t as GovernmentSignatureTemplateListItem).packageItemCount ?? 0),
  }))
}
...
/** @deprecated setGovernmentSignatureTemplateStatus(..., 'active') 사용 권장 */
export async function activateGovernmentSignatureTemplate(
  token: string,
  role: string | undefined,
  templateId: string,
  tenantGaId: number | null,
): Promise<void> {
  await setGovernmentSignatureTemplateStatus(token, role, templateId, 'active', tenantGaId)
}
...
export function buildStaffSignedPdfAbsUrl(sendSessionId: string, documentInstanceId: string): string {
  return resolveApiUrl(
    `/api/government-support/signatures/${encodeURIComponent(sendSessionId)}/documents/${encodeURIComponent(documentInstanceId)}/signed-pdf`,
  )
}

/** 발송 세션 단위 증빙 PDF (Bearer 인증) */
export function buildStaffEvidencePdfAbsUrl(sendSessionId: string): string {
  return resolveApiUrl(`/api/government-support/signatures/${encodeURIComponent(sendSessionId)}/evidence.pdf`)
}
