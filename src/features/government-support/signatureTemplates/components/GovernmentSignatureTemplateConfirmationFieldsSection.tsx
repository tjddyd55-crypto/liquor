...
import {
  ...
  listGovernmentSignatureTemplateConfirmationFields,
  updateGovernmentSignatureTemplateConfirmationField,
  type GovernmentSignatureTemplateConfirmationField,
  type GovernmentSignatureTemplateConfirmationFieldInputRole,
  type GovernmentSignatureTemplateConfirmationFieldInputType,
...
const INPUT_TYPE_OPTIONS: Array<{ value: GovernmentSignatureTemplateConfirmationFieldInputType; label: string }> = [
...
function toUpdateDraft(row: GovernmentSignatureTemplateConfirmationField): UpdateGovernmentSignatureTemplateConfirmationFieldInput {
  return {
    label: row.label,
...
export function GovernmentSignatureTemplateConfirmationFieldsSection({
  token,
  role,
  tenantGaId,
  templateId,
  ...
  const [editDraft, setEditDraft] = useState<UpdateGovernmentSignatureTemplateConfirmationFieldInput | null>(null)
  ...
  const startEdit = (row: GovernmentSignatureTemplateConfirmationField) => {
    setEditingId(row.id)
              ...
              <FormButton
                ...
                onClick={() =>
                  void (async () => {
                    ...
                    onError(null)
                    try {
                      const payload: CreateGovernmentSignatureTemplateConfirmationFieldInput = {
                        ...createDraft,
                        label,
                        fieldKey: createDraft.fieldKey?.trim() || undefined,
                        inputRole: normalizeInputRole(createDraft.inputRole),
                      }
                      await createGovernmentSignatureTemplateConfirmationField(token, role, templateId, payload, tenantGaId)
                      setAddOpen(false)
                      setCreateDraft(emptyCreateDraft())
                      await load()
                    } catch (e) {
                    ...
                    <FormButton
                      ...
                      onClick={() =>
                        void (async () => {
                          ...
                          onError(null)
                          try {
                            const payload: UpdateGovernmentSignatureTemplateConfirmationFieldInput = {
                              label,
                            ...
                            await updateGovernmentSignatureTemplateConfirmationField(
                              token,
                              role,
                              templateId,
                              row.id,
