...
import {
  ...
  duplicateGovernmentSignatureTemplate,
  fetchGovernmentSignatureTemplateDetail,
  patchGovernmentSignatureTemplate,
...
type Props = {
  ...
  templates: GovernmentSignatureTemplateListItem[]
  busy: boolean
  error: string | null
...
function templateDeleteEligibility(trow: GovernmentSignatureTemplateListItem): { canDelete: boolean; blockReason: string | null } {
...
function resolveTemplateMode(t: { templateMode?: string }): GovernmentSignatureTemplateMode {
  return t.templateMode === 'confirmation_only' ? 'confirmation_only' : 'coordinate_pdf'
}

function templateModeShortLabel(mode: GovernmentSignatureTemplateMode): string {
...
export function GovernmentSignatureTemplatePanel({
  token,
  role,
  tenantGaId,
  pdfTemplateId,
  ...
  const [detail, setDetail] = useState<GovernmentSignatureTemplateDetail | null>(null)
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const d = await fetchGovernmentSignatureTemplateDetail(token, role, id, tenantGaId)
        if (!cancelled) {
          setDetail(d)
        }
      } catch (e) {
  ...
  const openEdit = (row: GovernmentSignatureTemplateListItem) => {
    setEditTitle(row.title)
    setEditDescription(row.description ?? '')
    setModal({ kind: 'edit', templateId: row.id })
  }

  const openStatus = (row: GovernmentSignatureTemplateListItem) => {
    const s = row.status
                ...
                <article key={trow.id} className="contract-signature-console__template-card">
                  ...
                  <div className="contract-signature-console__template-card-actions">
                    ...
                    <FormButton
                      ...
                      onClick={() =>
                        void runOp(async () => {
                          await duplicateGovernmentSignatureTemplate(token, role, trow.id, tenantGaId)
                        })
                      }
                    >
                      복제
                      ...
                      <FormButton
                        ...
                        onClick={() =>
                          void (async () => {
                            ...
                            await runOp(async () => {
                              await setGovernmentSignatureTemplateStatus(token, role, trow.id, 'archived', tenantGaId)
                            })
                          })()
                        }
                      >
                    ...
                    <FormButton
                      ...
                      onClick={() =>
                        void (async () => {
                          ...
                          await runOp(async () => {
                            await deleteGovernmentSignatureTemplate(token, role, trow.id, tenantGaId)
                          })
                        })()
                      }
                    >
            ...
            {detailLoading ? (
              ...
              <>
                ...
                {resolveTemplateMode(detail) === 'confirmation_only' ? (
                  <GovernmentSignatureTemplateConfirmationFieldsSection
                    token={token}
                    role={role}
              ...
              <FormButton
                ...
                onClick={() =>
                  void (async () => {
                    ...
                    await runOp(async () => {
                      await patchGovernmentSignatureTemplate(
                        token,
                        role,
                        modal.templateId,
                      ...
                      if (
                        ...
                        await patchGovernmentSignatureTemplateFieldInputSettings(
                          token,
                          role,
                          modal.templateId,
                          { fieldSettings },
          ...
          <div className="contract-signature-console__detail-dialog" role="dialog" aria-modal="true" aria-label="상태 변경">
            ...
            <div className="contract-signature-console__detail-actions">
              ...
              <FormButton
                ...
                onClick={() =>
                  void runOp(async () => {
                    await setGovernmentSignatureTemplateStatus(token, role, modal.templateId, statusDraft, tenantGaId)
                    closeModal()
                  })
                }
              >
