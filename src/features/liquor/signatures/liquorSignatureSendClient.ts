...
export type UserLiquorSignatureTemplateItem = {
  id: string
  title: string
  ...
  senderFieldsForSend: LiquorSignatureSenderFieldDef[]
  sendable: boolean
}
...
export async function uploadUserLiquorSignatureSendAttachment(
  token: string,
  customerId: number,
  file: File,
): Promise<{
  ...
  const resolvedUrl = resolveApiUrl('/api/liquor/signatures/send-sessions/attachment-upload')
...
export async function listUserLiquorSignatureTemplates(
  token: string,
): Promise<UserLiquorSignatureTemplateItem[]> {
  const body = await apiRequest<{ templates?: UserLiquorSignatureTemplateItem[] }>(
    '/api/liquor/signatures/templates',
    { method: 'GET', token },
  )
  const raw = body as { templates?: UserLiquorSignatureTemplateItem[] }
 ...
 */
export async function listUserLiquorSignatureTemplateConfirmationFields(
  token: string,
  templateId: string,
): Promise<UserContractConfirmationFieldRow[]> {
  const body = await apiRequest<{ fields?: unknown[] }>(
    `/api/liquor/signatures/templates/${encodeURIComponent(templateId)}/confirmation-fields`,
    { method: 'GET', token },
  )
...
function dedupeLiquorSignatureSendHitsById(
...
export async function searchCustomersForLiquorSignatureSend(
  token: string,
  q: string,
  ...
  const body = await apiRequest<{ customers?: UserContractCustomerSearchHit[] }>(
    `/api/liquor/signatures/customers/search?${qs.toString()}`,
    { method: 'GET', token },
  )
  ...
  return dedupeLiquorSignatureSendHitsById(raw.customers)
}

export async function createUserLiquorSignatureSendSession(
  token: string,
  params: {
    customerId: number
    templateIds: string[]
  ...
  }>('/api/liquor/signatures/send-sessions', {
    method: 'POST',
    token,
    body: JSON.stringify({
...
export async function getUserLiquorSignatureSendSessionDetail(
  token: string,
  sendSessionId: string,
): Promise<SendSessionDetail> {
  const body = await apiRequest<{ sendSession?: SendSessionDetail }>(
    `/api/liquor/signatures/send-sessions/${encodeURIComponent(sendSessionId)}`,
    { method: 'GET', token },
  )
  const s = body.sendSession
  if (!s?.id || !s.linkCode) {
