...
import type { SendSessionDetail } from '../signatureTemplates/liquorSignatureTemplateClient'
...
export async function listUserSendSessions(
  ...
  const body = await apiRequest<{
    ...
    limit?: number
    offset?: number
  }>(`/api/liquor/signatures/send-sessions${suffix}`, { method: 'GET', token })
...
export async function cancelUserSendSession(token: string, sendSessionId: string): Promise<CancelUserSendSessionResult> {
  const body = await apiRequest<CancelUserSendSessionResult>(`/api/liquor/signatures/send-sessions/${encodeURIComponent(sendSessionId)}/cancel`, {
    method: 'PATCH',
    token,
    body: JSON.stringify({}),
  })
...
export function buildCustomerPublicSignUrl(linkCode: string): string {
  ...
  const path = `/liquor/sign/${encodeURIComponent(raw)}`
