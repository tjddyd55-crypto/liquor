/**
 * 전자서명 발송 — USER / GA_STAFF. 관리자 템플릿은 /liquor/signature-templates 에서만 관리.
 */
...
import '../signatureTemplates/liquor-signature-console.css'
...
import {
  createUserLiquorSignatureSendSession,
  getUserLiquorSignatureSendSessionDetail,
  listUserLiquorSignatureTemplates,
  listUserLiquorSignatureTemplateConfirmationFields,
  getContractCustomerSearchValidationMessage,
  searchCustomersForLiquorSignatureSend,
  CONTRACT_SEND_CONFIRMATION_MAX_ITEMS,
  CONTRACT_SEND_CONFIRMATION_MAX_LABEL_LEN,
  CONTRACT_SEND_ATTACHMENTS_MAX,
  uploadUserLiquorSignatureSendAttachment,
  type UserContractCustomerSearchHit,
  type UserLiquorSignatureTemplateItem,
  type UserContractConfirmationFieldRow,
} from './liquorSignatureSendClient'
 ...
 */
function liquorSignatureSendSessionDetailMatchesPick(
...
function sniffLiquorSignatureSendAttachmentMime(file: File): string {
...
function validateLiquorSignatureSendAttachmentFile(file: File): { ok: true; mime: string } | { ok: false; message: string } {
  if (file.size > ATTACHMENT_MAX_BYTES) {
    return { ok: false, message: '첨부파일은 파일당 최대 20MB까지 업로드할 수 있습니다.' }
  }
  const mime = sniffLiquorSignatureSendAttachmentMime(file)
...
export default function LiquorSignatureSendPage() {
  const { token } = useAuth()
  const t = token?.trim() ?? ''
  ...
  const [templates, setTemplates] = useState<UserLiquorSignatureTemplateItem[]>([])
  ...
  const reloadTemplates = useCallback(async () => {
    ...
    setBootError(null)
    try {
      const list = await listUserLiquorSignatureTemplates(t)
      setTemplates(list)
    } catch (e) {
  ...
  const executeCustomerSearch = useCallback(async () => {
    ...
    try {
      const hits = await searchCustomersForLiquorSignatureSend(t, trimmed)
  ...
  const refreshSessionDetail = useCallback(async () => {
    ...
    try {
      const next = await getUserLiquorSignatureSendSessionDetail(t, sid)
  ...
  useEffect(() => {
    ...
    setConfirmationFieldsError(null)
    void listUserLiquorSignatureTemplateConfirmationFields(t, selectedTemplateId)
      .then((rows) => {
        if (cancelled) {
          return
        }
  ...
  const onCreateSendSession = async () => {
    ...
    try {
      ...
      const created = await createUserLiquorSignatureSendSession(t, {
      ...
      const next = await getUserLiquorSignatureSendSessionDetail(t, created.id)
      setSessionDetail(next)
      setAttachmentDrafts([])
    } catch (e) {
  ...
  const scopedSendSessionDetail = useMemo((): SendSessionDetail | null => {
    if (!liquorSignatureSendSessionDetailMatchesPick(sessionDetail, selectedCustomer?.id, selectedTemplateId)) {
      return null
    }
  ...
  const processPickedAttachmentFiles = async (files: File[]) => {
    ...
    for (const file of files) {
      const checked = validateLiquorSignatureSendAttachmentFile(file)
      if (!checked.ok) {
        ...
        setAttachmentDrafts((prev) => {
          ...
          return [
            ...prev,
            {
              ...
              mimeType: sniffLiquorSignatureSendAttachmentMime(file) || file.type || 'application/octet-stream',
              sizeBytes: file.size,
              required: true,
              uploadStatus: 'error',
      ...
      try {
        const up = await uploadUserLiquorSignatureSendAttachment(t, selectedCustomer.id, file)
        setAttachmentDrafts((prev) =>
          prev.map((x) =>
            x.key === key
              ? {
