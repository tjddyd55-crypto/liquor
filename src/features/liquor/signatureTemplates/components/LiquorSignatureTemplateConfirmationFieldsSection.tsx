...
import {
  createLiquorSignatureTemplateConfirmationField,
  deleteLiquorSignatureTemplateConfirmationField,
  listLiquorSignatureTemplateConfirmationFields,
  updateLiquorSignatureTemplateConfirmationField,
  type LiquorSignatureTemplateConfirmationField,
  type LiquorSignatureTemplateConfirmationFieldInputRole,
  type LiquorSignatureTemplateConfirmationFieldInputType,
  type CreateLiquorSignatureTemplateConfirmationFieldInput,
  type UpdateLiquorSignatureTemplateConfirmationFieldInput,
} from '../liquorSignatureTemplateConfirmationFieldsClient'
...
const INPUT_TYPE_OPTIONS: Array<{ value: LiquorSignatureTemplateConfirmationFieldInputType; label: string }> = [
...
const INPUT_ROLE_OPTIONS: Array<{ value: LiquorSignatureTemplateConfirmationFieldInputRole; label: string }> = [
  { value: 'sender', label: '발송자 입력' },
  { value: 'customer', label: '고객 입력' },
]

function normalizeInputRole(raw: unknown): LiquorSignatureTemplateConfirmationFieldInputRole {
...
function emptyCreateDraft(): CreateLiquorSignatureTemplateConfirmationFieldInput {
  return {
    label: '',
    fieldKey: undefined,
    inputType: 'text',
...
function toUpdateDraft(row: LiquorSignatureTemplateConfirmationField): UpdateLiquorSignatureTemplateConfirmationFieldInput {
  return {
    label: row.label,
...
export function LiquorSignatureTemplateConfirmationFieldsSection({
  token,
  role,
  tenantGaId,
  templateId,
  ...
  const [items, setItems] = useState<LiquorSignatureTemplateConfirmationField[]>([])
  const [submitting, setSubmitting] = useState(false)
  const [addOpen, setAddOpen] = useState(false)
  const [createDraft, setCreateDraft] = useState(emptyCreateDraft)
  const [editingId, setEditingId] = useState<string | null>(null)
  const [editDraft, setEditDraft] = useState<UpdateLiquorSignatureTemplateConfirmationFieldInput | null>(null)

  const load = useCallback(async () => {
    setLoading(true)
    onError(null)
    try {
      const list = await listLiquorSignatureTemplateConfirmationFields(token, role, templateId, tenantGaId)
      setItems(list)
    } catch (e) {
  ...
  const startEdit = (row: LiquorSignatureTemplateConfirmationField) => {
    setEditingId(row.id)
              ...
              <FormButton
                ...
                onClick={() =>
                  void (async () => {
                    ...
                    onError(null)
                    try {
                      const payload: CreateLiquorSignatureTemplateConfirmationFieldInput = {
                        ...createDraft,
                        label,
                        fieldKey: createDraft.fieldKey?.trim() || undefined,
                        inputRole: normalizeInputRole(createDraft.inputRole),
                      }
                      await createLiquorSignatureTemplateConfirmationField(token, role, templateId, payload, tenantGaId)
                      setAddOpen(false)
                      setCreateDraft(emptyCreateDraft())
                      await load()
                    } catch (e) {
                    ...
                    <FormButton
                      ...
                      onClick={() =>
                        void (async () => {
                          ...
                          onError(null)
                          try {
                            const payload: UpdateLiquorSignatureTemplateConfirmationFieldInput = {
                              label,
                            ...
                            await updateLiquorSignatureTemplateConfirmationField(
                              token,
                              role,
                              templateId,
                              row.id,
                    ...
                    <FormButton
                      ...
                      onClick={() =>
                        void (async () => {
                          ...
                          onError(null)
                          try {
                            await deleteLiquorSignatureTemplateConfirmationField(token, role, templateId, row.id, tenantGaId)
                            await load()
                          } catch (e) {
