...
import type { LiquorSignatureTemplateDetail, LiquorSignatureTemplateListItem } from '../liquorSignatureTemplateClient'
import {
  createConfirmationOnlyLiquorSignatureTemplate,
  deleteLiquorSignatureTemplate,
  duplicateLiquorSignatureTemplate,
  fetchLiquorSignatureTemplateDetail,
  patchLiquorSignatureTemplate,
  patchLiquorSignatureTemplateFieldInputSettings,
  setLiquorSignatureTemplateStatus,
} from '../liquorSignatureTemplateClient'
import { LiquorSignatureTemplateConfirmationFieldsSection } from './LiquorSignatureTemplateConfirmationFieldsSection'
...
type Props = {
  ...
  templates: LiquorSignatureTemplateListItem[]
  busy: boolean
  error: string | null
...
function templateDeleteEligibility(trow: LiquorSignatureTemplateListItem): { canDelete: boolean; blockReason: string | null } {
...
function resolveTemplateMode(t: { templateMode?: string }): LiquorSignatureTemplateMode {
  return t.templateMode === 'confirmation_only' ? 'confirmation_only' : 'coordinate_pdf'
}

function templateModeShortLabel(mode: LiquorSignatureTemplateMode): string {
...
export function LiquorSignatureTemplatePanel({
  token,
  role,
  tenantGaId,
  pdfTemplateId,
  ...
  const [detail, setDetail] = useState<LiquorSignatureTemplateDetail | null>(null)
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const d = await fetchLiquorSignatureTemplateDetail(token, role, id, tenantGaId)
        if (!cancelled) {
          setDetail(d)
        }
      } catch (e) {
  ...
  const openEdit = (row: LiquorSignatureTemplateListItem) => {
    setEditTitle(row.title)
    setEditDescription(row.description ?? '')
    setModal({ kind: 'edit', templateId: row.id })
  }

  const openStatus = (row: LiquorSignatureTemplateListItem) => {
    const s = row.status
                ...
                <article key={trow.id} className="contract-signature-console__template-card">
                  ...
                  <div className="contract-signature-console__template-card-actions">
                    ...
                    <FormButton
                      ...
                      onClick={() =>
                        void runOp(async () => {
                          await duplicateLiquorSignatureTemplate(token, role, trow.id, tenantGaId)
                        })
                      }
                    >
                      복제
                      ...
                      <FormButton
                        ...
                        onClick={() =>
                          void (async () => {
                            ...
                            await runOp(async () => {
                              await setLiquorSignatureTemplateStatus(token, role, trow.id, 'archived', tenantGaId)
                            })
                          })()
                        }
                      >
                    ...
                    <FormButton
                      ...
                      onClick={() =>
                        void (async () => {
                          ...
                          await runOp(async () => {
                            await deleteLiquorSignatureTemplate(token, role, trow.id, tenantGaId)
                          })
                        })()
                      }
                    >
          ...
          <div className="contract-signature-console__detail-dialog" role="dialog" aria-modal="true" aria-label="템플릿 상세">
            ...
            {detailLoading ? (
              ...
              <dl className="contract-signature-console__detail-dl">
                ...
                <dd>{detail.liquorSignatureTemplateFieldsCount}</dd>
                <dt>설명</dt>
            ...
            {detailLoading ? (
              ...
              <>
                ...
                {resolveTemplateMode(detail) === 'confirmation_only' ? (
                  <LiquorSignatureTemplateConfirmationFieldsSection
                    token={token}
                    role={role}
              ...
              <FormButton
                ...
                onClick={() =>
                  void (async () => {
                    ...
                    await runOp(async () => {
                      await patchLiquorSignatureTemplate(
                        token,
                        role,
                        modal.templateId,
                      ...
                      if (
                        ...
                        await patchLiquorSignatureTemplateFieldInputSettings(
                          token,
                          role,
                          modal.templateId,
                          { fieldSettings },
          ...
          <div className="contract-signature-console__detail-dialog" role="dialog" aria-modal="true" aria-label="상태 변경">
            ...
            <div className="contract-signature-console__detail-actions">
              ...
              <FormButton
                ...
                onClick={() =>
                  void runOp(async () => {
                    await setLiquorSignatureTemplateStatus(token, role, modal.templateId, statusDraft, tenantGaId)
                    closeModal()
                  })
                }
              >
            ...
            <div className="contract-signature-console__detail-actions">
              ...
              <FormButton
                ...
                onClick={() => {
                  ...
                  void runOp(async () => {
                    await createConfirmationOnlyLiquorSignatureTemplate(token, role, {
                      title,
                      tenantGaId,
                    })
                    setConfirmOnlyCreateOpen(false)
