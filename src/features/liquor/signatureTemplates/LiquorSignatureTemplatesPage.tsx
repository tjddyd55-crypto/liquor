...
import './liquor-signature-console.css'
import { useAuth } from '../../auth/AuthProvider'
import { ApiError } from '../../../lib/apiClient'
import { FormButton } from '../../../components/form'
import { useMediaQuery } from '../../../hooks/useMediaQuery'
import { LiquorSignatureTemplatePanel } from './components/LiquorSignatureTemplatePanel'
import type { PdfPickRow } from './components/PdfTemplateSelector'
import { PdfTemplateSelector } from './components/PdfTemplateSelector'
import {
  countPdfFieldStats,
  createLiquorSignatureTemplateFromPdfTemplate,
  getPdfTemplateDetailForContractTest,
  listLiquorSignatureTemplates,
  listPdfTemplatesForContractTest,
  type LiquorSignatureTemplateListItem,
...
export default function LiquorSignatureTemplatesPage() {
  ...
  const [liquorSignatureTemplates, setLiquorSignatureTemplates] = useState<LiquorSignatureTemplateListItem[]>([])
  ...
  const visibleLiquorSignatureTemplates = useMemo(() => {
    if (selectedPdfId == null) {
      return liquorSignatureTemplates
    }
    return liquorSignatureTemplates.filter((x) => x.pdfTemplateId === selectedPdfId)
  }, [liquorSignatureTemplates, selectedPdfId])
  ...
  const reloadContracts = useCallback(async () => {
    ...
    try {
      const list = await listLiquorSignatureTemplates(t, role, tenantGaId)
      setLiquorSignatureTemplates(list)
    } catch (e) {
  ...
  const createTemplateFromSelectedPdf = useCallback(async () => {
    ...
    await createLiquorSignatureTemplateFromPdfTemplate(t, role, {
  ...
  const resolveCoordinateEditorHref =
    role === 'SUPER_ADMIN' ? (pdfTemplateId: number) => `/liquor/signature-templates/pdf/${pdfTemplateId}` : undefined
      ...
      <div className="contract-signature-console__container">
        ...
        <div className="contract-signature-console__notice" role="status">
          <ul>
            ...
            <li>관리자용 임시 발송·세션 조회 API(`/api/liquor/signature-templates/send-sessions`)는 호환용으로 유지될 수 있습니다.</li>
          </ul>
        </div>
        ...
        <section className="contract-signature-console__section">
          ...
          <LiquorSignatureTemplatePanel
            ...
            templates={visibleLiquorSignatureTemplates}
            busy={contractBusy}
