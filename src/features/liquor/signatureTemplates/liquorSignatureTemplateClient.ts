...
import {
  getLiquorPdfTemplate,
  listLiquorPdfTemplates,
} from './liquorPdfTemplateApiClient'
import type { PdfTemplateDetail, PdfTemplateSummary } from '../../pdf-engine/types'
import { searchCustomers } from '../../customers/api/customersApi'
import type { CustomerRecord } from '../../customers/domain/types'

export type LiquorSignatureTemplateMode = 'coordinate_pdf' | 'confirmation_only'

export type LiquorSignatureTemplateListItem = {
  id: string
  title: string
  ...
  status: string
  version: number
  templateMode: LiquorSignatureTemplateMode
  ...
  createdAt: string
  updatedAt: string
  /** liquor_signature_document_instances 건수(발송 이력 판단) */
  documentInstanceCount: number
  /** liquor_signature_package_items 건수 */
...
export type LiquorSignatureTemplateDetail = {
  id: string
  title: string
  description: string | null
  category: string | null
  ...
  pageCount: number | null
  gaId: number | null
  liquorSignatureTemplateFieldsCount: number
...
export type LiquorSignatureSendConfirmationItem = {
  id: string
  label: string
  required: boolean
  sortOrder: number
...
export type SendSessionDetail = {
  ...
  confirmationItems?: LiquorSignatureSendConfirmationItem[]
...
export async function listPdfTemplatesForContractTest(
  ...
  const { templates } = await listLiquorPdfTemplates(token)
...
export async function getPdfTemplateDetailForContractTest(
  ...
  return getLiquorPdfTemplate(token, id)
}
...
export async function listLiquorSignatureTemplates(
  token: string,
  role: string | undefined,
  tenantGaId: number | null,
): Promise<LiquorSignatureTemplateListItem[]> {
  const isSuper = role === 'SUPER_ADMIN'
  const qs = tenantQs(tenantGaId, isSuper)
  const body = await apiRequest<{ templates?: LiquorSignatureTemplateListItem[] }>(
    `/api/liquor/signature-templates/templates${qs}`,
    { method: 'GET', token },
  )
  const raw = body as { templates?: LiquorSignatureTemplateListItem[] }
  ...
  return raw.templates.map((t) => ({
    ...
    documentInstanceCount: Number((t as LiquorSignatureTemplateListItem).documentInstanceCount ?? 0),
    packageItemCount: Number((t as LiquorSignatureTemplateListItem).packageItemCount ?? 0),
  }))
}

export async function fetchLiquorSignatureTemplateDetail(
  token: string,
  role: string | undefined,
  templateId: string,
  tenantGaId: number | null,
): Promise<LiquorSignatureTemplateDetail> {
  const isSuper = role === 'SUPER_ADMIN'
  const qs = tenantQs(tenantGaId, isSuper)
  const body = await apiRequest<{ template?: LiquorSignatureTemplateDetail }>(
    `/api/liquor/signature-templates/templates/${encodeURIComponent(templateId)}${qs}`,
    { method: 'GET', token },
  )
  const tpl = (body as { template?: LiquorSignatureTemplateDetail }).template
  if (!tpl?.id) {
...
export async function patchLiquorSignatureTemplateFieldInputSettings(
  token: string,
  role: string | undefined,
  templateId: string,
  ...
  const qs = tenantQs(tenantGaId, isSuper)
  await apiRequest(
    `/api/liquor/signature-templates/templates/${encodeURIComponent(templateId)}/field-input-settings${qs}`,
    {
      method: 'PATCH',
      token,
...
export async function patchLiquorSignatureTemplate(
  token: string,
  role: string | undefined,
  templateId: string,
  ...
  const isSuper = role === 'SUPER_ADMIN'
  const qs = tenantQs(tenantGaId, isSuper)
  await apiRequest(`/api/liquor/signature-templates/templates/${encodeURIComponent(templateId)}${qs}`, {
    method: 'PATCH',
    token,
...
export async function createLiquorSignatureTemplateFromPdfTemplate(
  token: string,
  role: string | undefined,
  ...
  const body = await apiRequest<{ data?: { id?: string } }>(`/api/liquor/signature-templates/templates`, {
    method: 'POST',
    token,
    body: JSON.stringify({
      title,
...
export async function createConfirmationOnlyLiquorSignatureTemplate(
  token: string,
  role: string | undefined,
...
export async function setLiquorSignatureTemplateStatus(
  token: string,
  role: string | undefined,
  templateId: string,
  ...
  const isSuper = role === 'SUPER_ADMIN'
  const qs = tenantQs(tenantGaId, isSuper)
  await apiRequest(`/api/liquor/signature-templates/templates/${encodeURIComponent(templateId)}/status${qs}`, {
    method: 'PATCH',
    token,
...
/** @deprecated setLiquorSignatureTemplateStatus(..., 'active') 사용 권장 */
export async function activateLiquorSignatureTemplate(
  token: string,
  role: string | undefined,
  templateId: string,
  tenantGaId: number | null,
): Promise<void> {
  await setLiquorSignatureTemplateStatus(token, role, templateId, 'active', tenantGaId)
}

export async function deleteLiquorSignatureTemplate(
  token: string,
  role: string | undefined,
  templateId: string,
  tenantGaId: number | null,
...
export async function duplicateLiquorSignatureTemplate(
  token: string,
  role: string | undefined,
  templateId: string,
  tenantGaId: number | null,
  ...
  const body = await apiRequest<{ data?: { id?: string } }>(
    `/api/liquor/signature-templates/templates/${encodeURIComponent(templateId)}/duplicate${qs}`,
    {
      method: 'POST',
      token,
...
export async function createLiquorSignatureSendSession(
  token: string,
  role: string | undefined,
  params: {
    customerId: number
  ...
  const body = await apiRequest<{ sendSession?: CreateSendSessionResult }>(
    `/api/liquor/signature-templates/send-sessions`,
    {
      method: 'POST',
      token,
      body: JSON.stringify({
...
export async function getLiquorSignatureSendSessionDetail(
  token: string,
  role: string | undefined,
  ...
  const body = await apiRequest<{ sendSession?: SendSessionDetail }>(
    `/api/liquor/signature-templates/send-sessions/${encodeURIComponent(sendSessionId)}${qs}`,
    { method: 'GET', token },
  )
...
export function buildStaffSignedPdfAbsUrl(sendSessionId: string, documentInstanceId: string): string {
  return resolveApiUrl(
    `/api/liquor/signatures/send-sessions/${encodeURIComponent(sendSessionId)}/documents/${encodeURIComponent(documentInstanceId)}/signed-pdf`,
  )
}

/** 발송 세션 단위 증빙 PDF (Bearer 인증) */
export function buildStaffEvidencePdfAbsUrl(sendSessionId: string): string {
  return resolveApiUrl(`/api/liquor/signatures/send-sessions/${encodeURIComponent(sendSessionId)}/evidence.pdf`)
}
