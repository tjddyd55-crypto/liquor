...
import './liquor-signature-public.css'
import {
  ApiError,
  ...
  resolveLiquorSignedPdfAbsUrl,
  type ContractDocumentDetailPayload,
  type ContractPublicValueInput,
  type LiquorSignatureSendSessionAttachmentPublic,
} from './liquorSignaturePublicClient'
import { PublicPdfPreviewModal } from './components/PublicPdfPreviewModal'
import { LiquorSignatureAttachmentReviewModal } from './components/LiquorSignatureAttachmentReviewModal'
...
export default function LiquorSignDocumentPage() {
  ...
  const publicSignedPdfHref = useMemo(
    () => (paramsInvalid ? '' : resolveLiquorSignedPdfAbsUrl(linkCode, documentInstanceId)),
  ...
  const [attachmentModal, setAttachmentModal] = useState<LiquorSignatureSendSessionAttachmentPublic | null>(null)
  ...
  if (paramsInvalid) {
    body = (
      <div className="contract-public-sign-page__panel-danger-soft">
        ...
        <Link
          ...
          to={linkCode ? `/liquor/sign/${encodeURIComponent(linkCode)}` : '/'}
        >
          목록으로
        </Link>
      </div>
    ...
    body = (
      <div className="contract-public-sign-page__panel-danger-soft">
        ...
        <Link className="contract-public-sign-page__link contract-public-sign-page__link--after-block" to={`/liquor/sign/${encodeURIComponent(linkCode)}`}>
          목록으로
        </Link>
      </div>
    )
      ...
      if (detail.document.status === 'completed') {
        ...
        body = (
          <div className="contract-public-sign-page__stack">
            ...
            <Link
              className="contract-public-sign-page__link contract-public-sign-page__link--after-block"
              to={`/liquor/sign/${encodeURIComponent(linkCode)}`}
            >
              ← 문서 목록
            </Link>
          </div>
        ...
        <div className="contract-public-sign-page__stack">
          ...
          <LiquorSignatureAttachmentReviewModal
