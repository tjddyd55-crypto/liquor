...
export async function createTemplate(pool, input) {
  ...
  const { rows } = await q(
    `INSERT INTO liquor_pdf_templates
...
export async function updateTemplateMeta(pool, id, patch) {
  ...
  params.push(id)
  await pool.query(
    `UPDATE liquor_pdf_templates SET ${sets.join(', ')} WHERE id = $${params.length}`,
    params,
  )
}
...
export async function deleteTemplate(pool, id) {
  ...
  await pool.query(`DELETE FROM liquor_pdf_templates WHERE id = $1`, [id])
}
...
export async function replaceTemplateFields(pool, templateId, fields) {
  ...
  try {
    ...
    await client.query(`DELETE FROM liquor_pdf_template_fields WHERE template_id = $1`, [templateId])
    for (let i = 0; i < fields.length; i += 1) {
      ...
      await client.query(
        `INSERT INTO liquor_pdf_template_fields
    ...
    await client.query(`UPDATE liquor_pdf_templates SET updated_at = NOW() WHERE id = $1`, [templateId])
