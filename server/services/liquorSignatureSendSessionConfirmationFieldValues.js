 ...
 * @param {string} sendSessionId
 * @param {string} templateId liquor_signature_templates.id
 */
export async function listSendSessionConfirmationFieldValuesForPublic(poolOrClient, sendSessionId, templateId) {
  const r = await poolOrClient.query(
    ...
    FROM liquor_signature_template_confirmation_fields ctc
    LEFT JOIN liquor_signature_send_session_confirmation_field_values v
