...
/**
 * DB liquor_pdf_template_fields 행 → FieldSpec (스탬프 전용 — 좌표만 쓰고 inputRole 은 customer 로 통일)
 * @param {object} row
 * @param {number} idx
 */
...
function shouldIncludeFieldForStamp(row, settingsMap) {
  ...
  const eff = effectiveLiquorSignatureFieldRole(row, settingsMap.get(fk))
 ...
 * @param {{ excludeSignatures?: boolean, liquorSignatureTemplateId?: string | null }} [options]
...
export async function buildStampedPdfBufferFromInstance(executor, pdfTemplateId, valRows, options = {}) {
  ...
  const liquorSignatureTemplateId =
    options.liquorSignatureTemplateId != null && String(options.liquorSignatureTemplateId).trim() !== ''
      ? String(options.liquorSignatureTemplateId)
      : null
  ...
  /** @type {Map<string, { inputRole: import('./liquorSignatureTemplateFieldSettings.js').ContractFieldInputRole, fixedValue: string | null }> | null} */
  let settingsMap = null
  if (liquorSignatureTemplateId) {
    settingsMap = await loadContractFieldSettingsMap(executor, liquorSignatureTemplateId)
  }
  ...
  const templateBytes = await getLiquorPdfTemplateObject(String(tpl.storage_key))
