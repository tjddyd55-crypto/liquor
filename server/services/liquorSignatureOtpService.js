...
import {
  getLiquorSignatureOtpExpiresSeconds,
  getLiquorSignatureOtpMaxAttempts,
  getLiquorSignatureOtpMaxSendsPerSession,
  getLiquorSignatureOtpPepper,
  getLiquorSignatureOtpResendCooldownSeconds,
} from '../lib/liquorSignatureOtpConfig.js'
...
import { sendContractSelfSmsOtp } from './liquorSignatureSelfSmsSend.js'
 ...
 */
export async function liquorSignatureOtpSend(pool, opts) {
  ...
  try {
    ...
    const verifiedR = await client.query(
      `
      SELECT id FROM liquor_signature_identity_sessions
    ...
    const now = new Date()
    const cooldownSec = getLiquorSignatureOtpResendCooldownSeconds()
    const maxSends = getLiquorSignatureOtpMaxSendsPerSession()

    if (identityRow) {
      if (identityRow.status === 'failed' && Number(identityRow.otp_attempt_count ?? 0) >= getLiquorSignatureOtpMaxAttempts()) {
        await client.query('ROLLBACK')
        return {
          httpStatus: 429,
    ...
    const ttlSec = getLiquorSignatureOtpExpiresSeconds()
    ...
    if (!identityRow) {
      await client.query(
        `
        INSERT INTO liquor_signature_identity_sessions (
        ...
        VALUES (
          $1, $2, $3, 'self_sms', 'phone_possession', 'liquor_signature',
          'otp_sent', $4, $5, NOW(), $6,
          0, 1, $7, $8,
          NOW(), NOW()
        )
      ...
      await client.query(
        `
        UPDATE liquor_signature_identity_sessions
        SET
          status = 'otp_sent',
 ...
 */
export async function liquorSignatureOtpVerify(pool, opts) {
  const linkCode = String(opts.linkCode ?? '').trim()
  const clientIp = String(opts.clientIp ?? '')
  const userAgent = String(opts.userAgent ?? '').slice(0, 512)
  ...
  try {
    ...
    if (row.identity_session_id) {
      const done = await client.query(
        `SELECT status FROM liquor_signature_identity_sessions WHERE id = $1 LIMIT 1`,
    ...
    const maxAtt = getLiquorSignatureOtpMaxAttempts()
 ...
 * @param {string} linkCode
 */
export async function liquorSignatureOtpStatus(pool, linkCode) {
  ...
  let verified = false
  if (row.identity_session_id) {
    const r = await pool.query(`SELECT status FROM liquor_signature_identity_sessions WHERE id = $1 LIMIT 1`, [
      row.identity_session_id,
    ])
