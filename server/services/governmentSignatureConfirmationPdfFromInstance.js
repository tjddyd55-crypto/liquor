 ...
 *   profileDisplayNameMasked?: string | null,
...
export async function buildConfirmationCertificatePdfBuffer(p) {
  ...
  flushParagraph(`계약 템플릿명: ${displayOrDash(p.governmentSignatureTemplateTitle)}`)
  flushParagraph(`문서 인스턴스 ID: ${shortenId(p.documentInstanceId)}`)
  flushParagraph(`발송 세션 ID: ${shortenId(p.sendSessionId)}`)
  flushParagraph(`공개 링크 코드: ${shortenId(p.signToken, 16)}`)
  ...
  flushParagraph(`고객 표시명: ${displayOrDash(p.profileDisplayNameMasked)}`)
