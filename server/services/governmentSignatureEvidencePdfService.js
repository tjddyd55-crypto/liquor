...
import { listConfirmationItemsWithValues } from './governmentSignatureConfirmationItems.js'
import { listSendSessionAttachmentsPublic } from './governmentSignatureSendAttachments.js'
...
export async function buildSendSessionEvidencePdf(ctx) {
  ...
  const profileDisplayName = String(sessionRow.profile_display_name ?? '').trim() || '고객'
  const tplLabel =
  ...
  const downloadFilename = `${safeFilenameSeg(tplLabel)}_${safeFilenameSeg(profileDisplayName)}_전자서명증빙.pdf`
  ...
  flushParagraph(`고객명: ${profileDisplayName}`)
