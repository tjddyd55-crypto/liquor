...
import {
  ...
  getGovernmentSignatureOtpMaxSendsPerSession,
  getGovernmentSignatureOtpPepper,
  getGovernmentSignatureOtpResendCooldownSeconds,
 ...
 */
async function loadSendSessionWithCustomerPhone(client, signToken) {
  ...
  const r = await client.query(
    `
    SELECT css.*, p.phone AS profile_phone_raw
...
/**
 * @param {import('pg').Pool} pool
 * @param {{ signToken: string, clientIp: string, userAgent: string, body: unknown }} opts
 */
export async function governmentSignatureOtpSend(pool, opts) {
  const signToken = String(opts.signToken ?? '').trim()
  ...
  try {
    ...
    const row = await loadSendSessionWithCustomerPhone(client, signToken)
    if (!row) {
    ...
    const verifiedR = await client.query(
      `
      SELECT id FROM gov_signature_identity_sessions
    ...
    const now = new Date()
    const cooldownSec = getGovernmentSignatureOtpResendCooldownSeconds()
    const maxSends = getGovernmentSignatureOtpMaxSendsPerSession()

    if (identityRow) {
      if (identityRow.status === 'failed' && Number(identityRow.otp_attempt_count ?? 0) >= getGovernmentSignatureOtpMaxAttempts()) {
        await client.query('ROLLBACK')
        return {
          httpStatus: 429,
    ...
    const ttlSec = getGovernmentSignatureOtpExpiresSeconds()
    ...
    if (!identityRow) {
      await client.query(
        `
        INSERT INTO gov_signature_identity_sessions (
          id, send_session_id, profile_id, provider, level, purpose,
          status, target_phone_masked, otp_hash, otp_sent_at, otp_expires_at,
          otp_attempt_count, otp_send_count, ip_hash, user_agent,
          created_at, updated_at
        )
        VALUES (
          $1, $2, $3, 'self_sms', 'phone_possession', 'gov_signature',
          'otp_sent', $4, $5, NOW(), $6,
          0, 1, $7, $8,
          NOW(), NOW()
        )
        `,
        [identityId, row.id, row.profile_id, maskedPhone, otpHash, expiresAt, ipHash, userAgent],
      )
    } else {
      await client.query(
        `
        UPDATE gov_signature_identity_sessions
        SET
          status = 'otp_sent',
    ...
    const sms = await sendGovernmentSignatureSelfSmsOtp({
      phoneDigits: digits,
      code,
...
/**
 * @param {import('pg').Pool} pool
 * @param {{ signToken: string, codeRaw: string, clientIp: string, userAgent: string, body: unknown }} opts
 */
export async function governmentSignatureOtpVerify(pool, opts) {
  const signToken = String(opts.signToken ?? '').trim()
  const clientIp = String(opts.clientIp ?? '')
  const userAgent = String(opts.userAgent ?? '').slice(0, 512)
  ...
  try {
    ...
    if (row.identity_session_id) {
      const done = await client.query(
        `SELECT status FROM gov_signature_identity_sessions WHERE id = $1 LIMIT 1`,
    ...
    const maxAtt = getGovernmentSignatureOtpMaxAttempts()
 ...
 * @param {string} signToken
 */
export async function governmentSignatureOtpStatus(pool, signToken) {
  const row = await loadSendSessionWithCustomerPhone(pool, String(signToken ?? '').trim())
  if (!row) {
    return { httpStatus: 404, payload: { success: false, message: '발송 세션을 찾을 수 없습니다.' } }
  }
  ...
  let verified = false
  if (row.identity_session_id) {
    const r = await pool.query(`SELECT status FROM gov_signature_identity_sessions WHERE id = $1 LIMIT 1`, [
      row.identity_session_id,
    ])
