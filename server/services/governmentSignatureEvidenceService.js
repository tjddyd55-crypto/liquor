 ...
 */
export function hashGovernmentSignatureSendAttachmentsForEvidence(rows) {
  if (!rows || rows.length === 0) {
    return null
  }
  const sorted = [...rows].sort((a, b) => {
 ...
 *   governmentSignatureTemplatePdfHash: string | null,
 *   valuesHash: string,
 * }} src
 */
export function computeDocumentReferenceHash(src) {
  const hasDoc =
    src.originalPdfHash && String(src.originalPdfHash).trim()
      ...
      : src.governmentSignatureTemplatePdfHash && String(src.governmentSignatureTemplatePdfHash).trim()
        ? String(src.governmentSignatureTemplatePdfHash).trim()
        : null
  if (hasDoc) {
    return null
  }
 ...
 *   governmentSignatureTemplate: Record<string, unknown>,
 *   pdfTemplateId: number | null,
...
export async function insertSignatureEvidenceRow(client, req, input) {
  ...
  const ctpdf = input.governmentSignatureTemplate.pdf_hash
  const originalPdfHash = orig && String(orig).trim() ? String(orig).trim() : null
  const governmentSignatureTemplatePdfHash = ctpdf && String(ctpdf).trim() ? String(ctpdf).trim() : null
  const documentHash = originalPdfHash ?? governmentSignatureTemplatePdfHash
  ...
  const documentReferenceHash = computeDocumentReferenceHash({
    ...
    originalPdfHash,
    governmentSignatureTemplatePdfHash,
    valuesHash,
  })
  ...
  const profileId =
    input.sendSession.profile_id != null && input.sendSession.profile_id !== ''
      ? Number(input.sendSession.profile_id)
      : null
  ...
  const confRows = await client.query(
    ...
    SELECT
      ...
      v.checked_at
    FROM gov_signature_confirmation_items i
    LEFT JOIN gov_signature_confirmation_item_values v
  ...
  const attachmentsHash = hashGovernmentSignatureSendAttachmentsForEvidence(attRows.rows)
  ...
  await client.query(
    `
    INSERT INTO gov_signature_evidences (
      id,
      send_session_id,
