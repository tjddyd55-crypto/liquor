 ...
 */
export function hashLiquorSignatureSendAttachmentsForEvidence(rows) {
  if (!rows || rows.length === 0) {
    return null
  }
  const sorted = [...rows].sort((a, b) => {
 ...
 *   liquorSignatureTemplatePdfHash: string | null,
 *   valuesHash: string,
 * }} src
 */
export function computeDocumentReferenceHash(src) {
  const hasDoc =
    src.originalPdfHash && String(src.originalPdfHash).trim()
      ...
      : src.liquorSignatureTemplatePdfHash && String(src.liquorSignatureTemplatePdfHash).trim()
        ? String(src.liquorSignatureTemplatePdfHash).trim()
        : null
  if (hasDoc) {
    return null
  }
 ...
 *   liquorSignatureTemplate: Record<string, unknown>,
 *   pdfTemplateId: number | null,
...
export async function insertSignatureEvidenceRow(client, req, input) {
  ...
  const ctpdf = input.liquorSignatureTemplate.pdf_hash
  const originalPdfHash = orig && String(orig).trim() ? String(orig).trim() : null
  const liquorSignatureTemplatePdfHash = ctpdf && String(ctpdf).trim() ? String(ctpdf).trim() : null
  const documentHash = originalPdfHash ?? liquorSignatureTemplatePdfHash
  ...
  const documentReferenceHash = computeDocumentReferenceHash({
    ...
    originalPdfHash,
    liquorSignatureTemplatePdfHash,
    valuesHash,
  })
  ...
  const confRows = await client.query(
    ...
    SELECT
      ...
      v.checked_at
    FROM liquor_signature_confirmation_items i
    LEFT JOIN liquor_signature_confirmation_item_values v
  ...
  const attachmentsHash = hashLiquorSignatureSendAttachmentsForEvidence(attRows.rows)
  ...
  await client.query(
    `
    INSERT INTO liquor_signature_evidences (
      id,
      send_session_id,
