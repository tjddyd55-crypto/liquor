...
function shouldIncludeFieldForStamp(row, settingsMap) {
  ...
  const eff = effectiveGovSignatureFieldRole(row, settingsMap.get(fk))
 ...
 * @param {{ excludeSignatures?: boolean, governmentSignatureTemplateId?: string | null }} [options]
...
export async function buildStampedPdfBufferFromInstance(executor, pdfTemplateId, valRows, options = {}) {
  ...
  const governmentSignatureTemplateId =
    options.governmentSignatureTemplateId != null && String(options.governmentSignatureTemplateId).trim() !== ''
      ? String(options.governmentSignatureTemplateId)
      : null
  ...
  /** @type {Map<string, { inputRole: import('./governmentSignatureTemplateFieldSettings.js').ContractFieldInputRole, fixedValue: string | null }> | null} */
  let settingsMap = null
  if (governmentSignatureTemplateId) {
    settingsMap = await loadContractFieldSettingsMap(executor, governmentSignatureTemplateId)
  }
