...
} from './governmentSignatureTemplateFieldSettings.js'
import { normalizeContractFieldStoredValue } from './governmentSignatureFieldValueNormalize.js'

/**
 * 설계사(sender) 입력 값을 검증하고 gov_signature_document_values 행 목록(SQL 파라미터)을 만든다.
 *
 ...
 */
export async function assertSenderFieldValuesFilled(db, governmentSignatureTemplateId, pdfTemplateId, senderValuesByFieldKey) {
  const fields = await listFields(db, pdfTemplateId)
  const settingsMap = await loadContractFieldSettingsMap(db, governmentSignatureTemplateId)
  const missed = []
 ...
 * @param {Record<string, unknown>} senderValuesByFieldKey
 */
export async function insertGovSenderPrefillDocumentValues(
  client,
  documentInstanceId,
  ...
  const settingsMap = await loadContractFieldSettingsMap(client, governmentSignatureTemplateId)
  for (const f of fields) {
    const fk = String(f.field_key)
    const role = effectiveGovSignatureFieldRole(f, settingsMap.get(fk))
    if (role !== 'sender' || String(f.field_type) === 'signature') {
 ...
 * @param {string[]} governmentSignatureTemplateIds ct_ 포함 id
 */
export function senderValuesByGovernmentSignatureTemplates(senderRoot, governmentSignatureTemplateIds) {
  /** @type {Map<string, Record<string, unknown>>} */
  const map = new Map()
  const ids = governmentSignatureTemplateIds.map((id) => String(id))
  for (const id of ids) {
    map.set(id, {})
  }
  ...
  const topKeys = Object.keys(root).filter((k) => typeof k === 'string' && k.startsWith('gst_'))
  if (topKeys.length > 0) {
    for (const id of ids) {
