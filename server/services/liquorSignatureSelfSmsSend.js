...
function liquorSignatureOtpSmsMockEnabled() {
  return String(process.env.LIQUOR_SIGNATURE_OTP_SMS_MOCK ?? '').trim().toLowerCase() === 'true'
}

/**
 * 계약서 지정번호 OTP 문자 발송.
 * - 운영: mock 금지. 기존 smsService(알리고/HTTP 게이트웨이) 사용.
 * - 비운영: LIQUOR_SIGNATURE_OTP_SMS_MOCK=true 또는 프로바이더 미설정 시 마스킹 번호 + OTP만 로그(운영에서는 절대 원문 로그 없음).
 *
...
export async function sendContractSelfSmsOtp(p) {
  ...
  const purpose = String(p.purpose ?? 'liquor_signature')
  ...
  if (RUNNING_IN_PRODUCTION && liquorSignatureOtpSmsMockEnabled()) {
    console.error('[contract OTP SMS] LIQUOR_SIGNATURE_OTP_SMS_MOCK must not be enabled in production')
  ...
  if (!RUNNING_IN_PRODUCTION && (liquorSignatureOtpSmsMockEnabled() || !isSmsProviderConfigured())) {
    console.log('[contract OTP SMS mock]', { toMasked: masked, expiresHint: 'see LIQUOR_SIGNATURE_OTP_EXPIRES_SECONDS', purpose })
