...
import {
  ...
  hashGovernmentSignatureSendAttachmentsForEvidence,
  pickSignatureAggregation,
  sha256Hex,
  stableStringify,
  truncateUserAgent,
} from './governmentSignatureEvidenceService.js'
 ...
 * @param {Array<Record<string, unknown>>} documentValueRows gov_signature_document_values
 */
...
export function buildConfirmationOnlyEvidenceReferencePayload(p) {
  return {
    ...
    signToken: p.signToken == null || String(p.signToken).trim() === '' ? null : String(p.signToken).trim(),
 ...
 *   governmentSignatureTemplateTitle: string,
...
export async function insertConfirmationOnlySignatureEvidenceRow(client, req, input) {
  ...
  const templateName = String(input.governmentSignatureTemplateTitle ?? '').trim() || '—'
  const signToken =
    input.sendSession.sign_token != null && String(input.sendSession.sign_token).trim()
      ? String(input.sendSession.sign_token).trim()
      : null
  ...
  const attachmentsHash = hashGovernmentSignatureSendAttachmentsForEvidence(attRowsDb.rows)
