...
import { normalizeContractFieldStoredValue } from './governmentSignatureFieldValueNormalize.js'
 ...
 */
export function normalizeGovSignatureFieldInputRole(raw) {
 ...
 * @param {string} governmentSignatureTemplateId
 * @returns {Promise<Map<string, { inputRole: ContractFieldInputRole, fixedValue: string | null }>>}
 */
export async function loadContractFieldSettingsMap(db, governmentSignatureTemplateId) {
  ...
  for (const row of r.rows) {
    ...
    if (!fk) continue
    m.set(fk, {
      inputRole: normalizeGovSignatureFieldInputRole(row.input_role),
 ...
 * @returns {ContractFieldInputRole}
 */
export function effectiveGovSignatureFieldRole(pdfRow, setting) {
  if (setting) {
    const ft = String(pdfRow?.field_type ?? '')
    if (ft === 'signature') {
      return 'customer'
    }
    return normalizeGovSignatureFieldInputRole(setting.inputRole)
  }
 ...
 */
export async function seedGovernmentSignatureTemplateFieldSettings(client, governmentSignatureTemplateId, pdfTemplateId) {
  ...
  for (const f of fields) {
    ...
    await client.query(
      ...
      `,
      [governmentSignatureTemplateId, fk, inputRole, fixedValue],
    )
  }
}
 ...
 * @param {number} pdfTemplateId
 */
export async function reconcileGovSignatureFieldSettingsAfterPdfSave(client, pdfTemplateId) {
  const fields = await listFields(client, pdfTemplateId)
  const keys = new Set(fields.map((f) => String(f.field_key)).filter(Boolean))
  const tr = await client.query(`SELECT id FROM gov_signature_templates WHERE pdf_template_id = $1`, [pdfTemplateId])
  for (const row of tr.rows) {
    const tid = String(row.id)
    const keyArr = [...keys]
    if (keyArr.length === 0) {
      await client.query(`DELETE FROM gov_signature_template_field_settings WHERE template_id = $1`, [tid])
      continue
    }
    await client.query(
      `DELETE FROM gov_signature_template_field_settings WHERE template_id = $1 AND NOT (field_key = ANY($2::text[]))`,
      [tid, keyArr],
    )
    for (const f of fields) {
      const fk = String(f.field_key ?? '')
 ...
 */
export async function assertGovSignatureFieldSettingsValidForActivate(db, governmentSignatureTemplateId, pdfTemplateId) {
  const pdfFields = await listFields(db, pdfTemplateId)
  const map = await loadContractFieldSettingsMap(db, governmentSignatureTemplateId)
  for (const f of pdfFields) {
    const fk = String(f.field_key ?? '')
    const st = map.get(fk)
    const role = effectiveGovSignatureFieldRole(f, st)
 ...
 * @param {number} pdfTemplateId
 */
export async function insertFixedPrefillDocumentValues(client, documentInstanceId, governmentSignatureTemplateId, pdfTemplateId) {
  const pdfFields = await listFields(client, pdfTemplateId)
  const map = await loadContractFieldSettingsMap(client, governmentSignatureTemplateId)
  for (const f of pdfFields) {
    const fk = String(f.field_key ?? '')
    const ft = String(f.field_type ?? '')
 ...
 */
export function listSenderFieldsForGovernmentSignatureTemplate(pdfFieldRows, settingsMap) {
  ...
  for (const row of pdfFieldRows) {
    ...
    const role = effectiveGovSignatureFieldRole(row, settingsMap.get(fk))
