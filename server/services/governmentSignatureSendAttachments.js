...
/**
 * 발송 세션에 첨부를 연결한다. 파일은 발송자 소유이며 profile_id 가 비었거나 동일 고객이어야 한다.
 ...
 */
export async function insertSendSessionAttachmentsForSend(client, sendSessionId, profileId, userId, ownerUserId, items) {
  for (const it of items) {
    const lock = await client.query(
      `
      SELECT id, original_name, display_name, mime_type, file_size, file_path, profile_id
      FROM files
      WHERE id = $1::bigint
        AND user_id = $2
        AND owner_user_id = $3
        AND (profile_id IS NULL OR profile_id = $4::int)
      FOR UPDATE
      `,
      [it.fileId, userId, ownerUserId, profileId],
    )
    if (!lock.rowCount) {
    ...
    await client.query(
      `
      INSERT INTO gov_signature_attachments (
        id,
        send_session_id,
        file_id,
        display_filename,
    ...
    await client.query(
      `
      UPDATE files
      SET profile_id = COALESCE(profile_id, $2::int)
      WHERE id = $1::bigint
      `,
      [it.fileId, profileId],
    )
...
export async function loadSendSessionAttachmentRow(executor, sendSessionId, attachmentId) {
  const r = await executor.query(
    ...
    SELECT
      ...
      a.confirmed,
      a.confirmed_at
    FROM gov_signature_attachments a
