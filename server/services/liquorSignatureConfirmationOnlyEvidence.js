...
import {
  ...
  hashLiquorSignatureSendAttachmentsForEvidence,
  pickSignatureAggregation,
  sha256Hex,
  stableStringify,
  truncateUserAgent,
} from './liquorSignatureEvidenceService.js'
 ...
 * @param {Array<Record<string, unknown>>} documentValueRows liquor_signature_document_values
 */
 ...
 *   liquorSignatureTemplateTitle: string,
...
export async function insertConfirmationOnlySignatureEvidenceRow(client, req, input) {
  ...
  const templateName = String(input.liquorSignatureTemplateTitle ?? '').trim() || '—'
  const linkCode =
  ...
  const attachmentsHash = hashLiquorSignatureSendAttachmentsForEvidence(attRowsDb.rows)
