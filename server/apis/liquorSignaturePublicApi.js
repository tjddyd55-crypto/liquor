...
import { decryptContractTargetPhoneBlob } from '../lib/liquorSignatureStoredPhone.js'
import { consentGetBuffer, consentPutObject } from '../lib/consentStorage.js'
import { buildLiquorSignatureObjectKey } from '../lib/liquorSignatureStorage.js'
...
import { getTemplateById, listFields } from '../pdf-engine/repository/liquorPdfTemplateRepo.js'
import { getLiquorPdfTemplateObject } from '../pdf-engine/storage/liquorPdfTemplateStorage.js'
import { insertSignatureEvidenceRow, loadVerifiedIdentitySession } from '../services/liquorSignatureEvidenceService.js'
import { normalizeContractFieldStoredValue } from '../services/liquorSignatureFieldValueNormalize.js'
import { insertConfirmationOnlySignatureEvidenceRow } from '../services/liquorSignatureConfirmationOnlyEvidence.js'
import { buildConfirmationCertificatePdfBuffer } from '../services/liquorSignatureConfirmationPdfFromInstance.js'
import { buildStampedPdfBufferFromInstance } from '../services/liquorSignatureStampedPdfFromInstance.js'
import {
...
} from '../services/liquorSignatureSendAttachments.js'
import { listSendSessionConfirmationFieldValuesForPublic } from '../services/liquorSignatureSendSessionConfirmationFieldValues.js'
...
async function loadLatestIdentityStatus(pool, sendSessionId) {
  const r = await pool.query(
    `
    SELECT status
    FROM liquor_signature_identity_sessions
...
async function loadValueRows(db, documentInstanceId) {
  const r = await db.query(
    ...
    FROM liquor_signature_document_values
...
async function resolveMutationBase(pool, linkCode, documentInstanceId, mutationKind = 'values') {
  ...
  const docR = await pool.query(
    `SELECT * FROM liquor_signature_document_instances WHERE id = $1 AND send_session_id = $2 LIMIT 1`,
  ...
  const tmR = await pool.query(
    `SELECT COALESCE(template_mode, 'coordinate_pdf') AS template_mode FROM liquor_signature_templates WHERE id = $1 LIMIT 1`,
    [doc.template_id],
  )
...
async function upsertDocumentValue(client, documentInstanceId, fieldRow, valueText, valueFileId, valueHash) {
  ...
  const ex = await client.query(
    `SELECT id FROM liquor_signature_document_values WHERE document_instance_id = $1 AND field_key = $2 LIMIT 1`,
    [documentInstanceId, fk],
  )
  if (ex.rowCount > 0) {
    await client.query(
      `
      UPDATE liquor_signature_document_values
      SET
        field_id = $1,
        field_type = $2,
        value_text = $3,
...
async function syncDocStatusAfterSign(client, pdfTemplateId, documentInstanceId, liquorSignatureTemplateId) {
  if (pdfTemplateId == null) {
    return
  }
  const fields = await listFields(client, Number(pdfTemplateId))
  const settingsMap = liquorSignatureTemplateId
    ? await loadContractFieldSettingsMap(client, String(liquorSignatureTemplateId))
    : new Map()
  const sigFields = fields.filter(
    (f) =>
      String(f.field_type) === 'signature' &&
      effectiveLiquorSignatureFieldRole(f, settingsMap.get(String(f.field_key))) === 'customer',
  )
  if (sigFields.length === 0) {
    await client.query(`UPDATE liquor_signature_document_instances SET status = 'signing', updated_at = NOW() WHERE id = $1`, [
      documentInstanceId,
    ])
    return
  }
  const vals = await client.query(
    `SELECT field_id, value_file_id FROM liquor_signature_document_values WHERE document_instance_id = $1`,
    [documentInstanceId],
  )
  ...
  await client.query(
    `UPDATE liquor_signature_document_instances SET status = $1, updated_at = NOW() WHERE id = $2`,
...
async function maybeCompleteSendSession(client, sendSessionId) {
  const q = await client.query(
    `SELECT required, status FROM liquor_signature_document_instances WHERE send_session_id = $1`,
    [sendSessionId],
  )
  ...
  if (ok) {
    await client.query(
      `
      UPDATE liquor_signature_send_sessions
      SET
        status = 'completed',
...
async function insertFinalSignedPdfFileRow(client, p) {
  const fileSuffix =
    p.storageFileName && String(p.storageFileName).trim()
      ...
      : 'final-signed.pdf'
  const storageKey = buildLiquorSignatureObjectKey(`sessions/${p.sessionId}/documents/${p.docId}/${fileSuffix}`)
...
function fieldRowToPublicDto(row, settingsMap) {
  ...
  const role = effectiveLiquorSignatureFieldRole(row, st)
...
async function loadSendSessionRow(pool, linkCode) {
  ...
  const r = await pool.query(
    ...
    FROM liquor_signature_send_sessions css
 ...
 */
export function registerLiquorSignaturePublicApi(apiRouter, ctx) {
  const { pool, handleDbError } = ctx

  apiRouter.post('/liquor/signatures/public/:linkCode/documents/:documentInstanceId/complete', async (req, res) => {
    try {
      ...
      const docMeta = await pool.query(
        ...
        FROM liquor_signature_document_instances cdi
        INNER JOIN liquor_signature_templates ct ON ct.id = cdi.template_id
      ...
      const liquorSignatureTemplatePdfHash = dm0.contract_template_pdf_hash ?? null
      const liquorSignatureTemplateIdStr = String(dm0.template_id)
      const liquorSignatureTemplateTitle = String(dm0.contract_template_title ?? '').trim() || '—'
      ...
      if (templateMode !== 'confirmation_only') {
        ...
        const settingsMapPre = await loadContractFieldSettingsMap(pool, liquorSignatureTemplateIdStr)
        ...
        const confirmationOnlyCompleteGate = await assertConfirmationOnlyPublicReadyForComplete(pool, {
          ...
          templateId: liquorSignatureTemplateIdStr,
        })
      ...
      try {
        ...
        const lockR = await client.query(
          `SELECT * FROM liquor_signature_document_instances WHERE id = $1 AND send_session_id = $2 FOR UPDATE`,
          [docId, session.id],
        )
        if (!lockR.rowCount) {
        ...
        if (String(docLocked.status ?? '') === 'completed') {
          if (templateMode === 'confirmation_only' && docLocked.signed_pdf_file_id) {
            ...
            const completedAtIsoDup = docLocked.completed_at
              ...
              : null
            const signedPdfPathDup = `/api/liquor/signatures/public/${encodeURIComponent(linkCode)}/documents/${encodeURIComponent(docId)}/signed-pdf`
            res.status(200).json({
              success: true,
              data: {
                status: 'completed',
        ...
        const confQ = await client.query(
          ...
          FROM liquor_signature_confirmation_items
          WHERE send_session_id = $1
        ...
        const attMissR = await client.query(
          ...
          FROM liquor_signature_send_session_attachments
          WHERE send_session_id = $1
            AND required = true
        ...
        if (templateMode === 'confirmation_only') {
          ...
          const settingsMapTx = await loadContractFieldSettingsMap(client, liquorSignatureTemplateIdStr)
          ...
          if (fileUserId && gaIdForFile != null) {
            try {
              const stamped = await buildStampedPdfBufferFromInstance(client, pdfTid, valRows, {
                liquorSignatureTemplateId: liquorSignatureTemplateIdStr,
              })
              const insPdf = await insertFinalSignedPdfFileRow(client, {
                userId: fileUserId,
                gaId: Number(gaIdForFile),
          ...
          try {
            await insertSignatureEvidenceRow(client, req, {
              sendSession: session,
              documentInstance: docLocked,
              liquorSignatureTemplate: { pdf_hash: liquorSignatureTemplatePdfHash },
        ...
        const completedRes = await client.query(
          `
          UPDATE liquor_signature_document_instances
          SET
            status = 'completed',
            completed_at = NOW(),
  ...
  apiRouter.get('/liquor/signatures/public/:linkCode/documents/:documentInstanceId/confirmation-signature', async (req, res) => {
    try {
      const linkCode = String(req.params.linkCode ?? '').trim()
      const docId = String(req.params.documentInstanceId ?? '').trim()
      const row = await loadSendSessionRow(pool, linkCode)
  ...
  apiRouter.post('/liquor/signatures/public/:linkCode/documents/:documentInstanceId/sign', async (req, res) => {
    try {
      const linkCode = String(req.params.linkCode ?? '').trim()
      const docId = String(req.params.documentInstanceId ?? '').trim()
      ...
      const liquorSignatureTemplateModeSign = String(docMeta.rows[0].contract_template_mode ?? 'coordinate_pdf')

      if (liquorSignatureTemplateModeSign === 'confirmation_only') {
        ...
        const storageKeyCo = buildLiquorSignatureObjectKey(`sessions/${session.id}/documents/${docId}/signature/${CONFIRMATION_ONLY_SIGNATURE_FIELD_KEY}.png`)
        ...
        try {
          ...
          await clientCo.query(
            `UPDATE liquor_signature_document_instances SET status = 'signed', updated_at = NOW() WHERE id = $1`,
            [docId],
          )
          await clientCo.query(
            `
      ...
      const liquorSignatureTemplateIdSign = String(docMeta.rows[0].template_id)
      const settingsMapSign = await loadContractFieldSettingsMap(pool, liquorSignatureTemplateIdSign)
      ...
      const sigFields = rawFields.filter(
        (f) =>
          String(f.field_type) === 'signature' &&
          effectiveLiquorSignatureFieldRole(f, settingsMapSign.get(String(f.field_key))) === 'customer',
      )
      if (sigFields.length === 0) {
      ...
      const storageKey = buildLiquorSignatureObjectKey(`sessions/${session.id}/documents/${docId}/signature/${targetField.id}.png`)
      ...
      try {
        ...
        await syncDocStatusAfterSign(client, pdfTid, docId, liquorSignatureTemplateIdSign)
        await client.query(
          `
          UPDATE liquor_signature_send_sessions
          SET
  ...
  apiRouter.post('/liquor/signatures/public/:linkCode/documents/:documentInstanceId/values', async (req, res) => {
    try {
      const linkCode = String(req.params.linkCode ?? '').trim()
      const docId = String(req.params.documentInstanceId ?? '').trim()
      ...
      const liquorSignatureTemplateMode = String(docMeta.rows[0].contract_template_mode ?? 'coordinate_pdf')

      if (liquorSignatureTemplateMode === 'confirmation_only') {
          ...
          if (confirmationFieldInputMap.size > 0) {
            for (const [fieldKey, valueText] of confirmationFieldInputMap.entries()) {
              const ex = await client.query(
                `
                SELECT id
                FROM liquor_signature_send_session_confirmation_field_values
              ...
              if (ex.rowCount > 0) {
                await client.query(
                  `
                  UPDATE liquor_signature_send_session_confirmation_field_values
                ...
                await client.query(
                  `
                  INSERT INTO liquor_signature_send_session_confirmation_field_values (
      ...
      const rawFields = await listFields(pool, Number(pdfTid))
      const liquorSignatureTemplateIdForValues = String(docMeta.rows[0].template_id)
      const { byId } = fieldMapsFromRows(rawFields)
      const client = await pool.connect()
      try {
        await client.query('BEGIN')
        const settingsMapPost = await loadContractFieldSettingsMap(client, liquorSignatureTemplateIdForValues)
        for (const item of bodyVals) {
          ...
          const role = effectiveLiquorSignatureFieldRole(fieldRow, settingsMapPost.get(String(fieldRow.field_key)))
  ...
  apiRouter.get('/liquor/signatures/public/:linkCode/documents/:documentInstanceId/pdf', async (req, res) => {
    try {
      const linkCode = String(req.params.linkCode ?? '').trim()
      const docId = String(req.params.documentInstanceId ?? '').trim()
      const row = await loadSendSessionRow(pool, linkCode)
      ...
      const buf = await getLiquorPdfTemplateObject(String(tpl.storage_key))
      res.setHeader('Content-Type', 'application/pdf')
      res.setHeader('Cache-Control', 'private, no-store')
  ...
  apiRouter.get('/liquor/signatures/public/:linkCode/documents/:documentInstanceId/rendered-pdf', async (req, res) => {
    try {
      const linkCode = String(req.params.linkCode ?? '').trim()
      const docId = String(req.params.documentInstanceId ?? '').trim()
      const row = await loadSendSessionRow(pool, linkCode)
      ...
      const pdfTid = docR.rows[0].pdf_template_id
      const liquorSignatureTemplateIdRendered = String(docR.rows[0].template_id)
      if (pdfTid == null) {
        res.status(404).json({ success: false, message: 'PDF 템플릿이 연결되어 있지 않습니다.' })
        return
      }
      ...
      try {
        stamped = await buildStampedPdfBufferFromInstance(pool, pdfTid, valRows, {
          excludeSignatures,
          liquorSignatureTemplateId: liquorSignatureTemplateIdRendered,
        })
      } catch (rendErr) {
        if (process.env.NODE_ENV !== 'production') {
  ...
  apiRouter.get('/liquor/signatures/public/:linkCode/documents/:documentInstanceId/signed-pdf', async (req, res) => {
    try {
      const linkCode = String(req.params.linkCode ?? '').trim()
      const docId = String(req.params.documentInstanceId ?? '').trim()
      const row = await loadSendSessionRow(pool, linkCode)
  ...
  apiRouter.get('/liquor/signatures/public/:linkCode/attachments/:attachmentId/view', async (req, res) => {
    try {
      const linkCode = String(req.params.linkCode ?? '').trim()
  ...
  apiRouter.post('/liquor/signatures/public/:linkCode/attachments/:attachmentId/confirm', async (req, res) => {
    try {
      const linkCode = String(req.params.linkCode ?? '').trim()
      const attachmentId = String(req.params.attachmentId ?? '').trim()
      const row = await loadSendSessionRow(pool, linkCode)
      ...
      const upd = await pool.query(
        `
        UPDATE liquor_signature_send_session_attachments
        SET
          viewed = true,
  ...
  apiRouter.get('/liquor/signatures/public/:linkCode/documents/:documentInstanceId', async (req, res) => {
    try {
      const linkCode = String(req.params.linkCode ?? '').trim()
      const docId = String(req.params.documentInstanceId ?? '').trim()
      const row = await loadSendSessionRow(pool, linkCode)
      ...
      if (String(doc.status ?? '') === 'pending') {
        await pool.query(
          `UPDATE liquor_signature_document_instances SET status = 'viewed', updated_at = NOW() WHERE id = $1`,
          [docId],
        )
        doc.status = 'viewed'
      }
      const liquorSignatureTemplateMode = String(doc.contract_template_mode ?? 'coordinate_pdf')
      ...
      if (liquorSignatureTemplateMode === 'confirmation_only') {
        ...
        const previewPath = hasSig
          ? `/api/liquor/signatures/public/${encodeURIComponent(linkCode)}/documents/${encodeURIComponent(docId)}/confirmation-signature`
          : null
      ...
      const pdfPreviewPath = `/api/liquor/signatures/public/${encodeURIComponent(linkCode)}/documents/${encodeURIComponent(docId)}/pdf`
  ...
  apiRouter.get('/liquor/signatures/public/:linkCode/documents', async (req, res) => {
    try {
      const linkCode = String(req.params.linkCode ?? '').trim()
      const row = await loadSendSessionRow(pool, linkCode)
      if (!row) {
  ...
  apiRouter.post('/liquor/signatures/public/:linkCode/open', async (req, res) => {
    try {
      const linkCode = String(req.params.linkCode ?? '').trim()
      const row = await loadSendSessionRow(pool, linkCode)
      if (!row) {
  ...
  apiRouter.get('/liquor/signatures/public/:linkCode', async (req, res) => {
    try {
      const linkCode = String(req.params.linkCode ?? '').trim()
      const row = await loadSendSessionRow(pool, linkCode)
      if (!row) {
