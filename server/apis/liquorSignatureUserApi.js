...
import {
  getAuthUserId,
  assertLiquorSignatureTemplateAccess,
  assertLiquorConfirmationOnlyTemplateRow,
  mapConfirmationFieldRow,
  buildTargetPhoneSnapshot,
  generateUniqueLinkCode,
  parseTemplateIdsArray,
} from './liquorSignatureTemplateApi.js'
import {
...
import {
  ...
  listSenderFieldsForLiquorSignatureTemplate,
...
} from '../services/liquorSignatureEvidencePdfService.js'
import {
...
import { insertSendSessionConfirmationFieldValues } from '../services/liquorSignatureSendSessionConfirmationFieldValues.js'
 ...
 * @param {unknown} err
 */
function logLiquorSignatureSendSessionFailure(ctx, err) {
  const e = err instanceof Error ? err : new Error(String(err))
  if (VERBOSE_CONTRACT_SEND_LOGS) {
    console.error('[liquor/signatures/send-sessions]', {
      route: ctx.route,
      userId: ctx.userId,
      gaId: ctx.gaId,
...
function mapSendSessionCreateError(err) {
  ...
  if (msg.includes('LIQUOR_SIGNATURE_OTP_PEPPER') || msg.includes('[contract OTP]')) {
    return {
      status: 503,
      code: 'missing_contract_otp_pepper',
      message:
        '전자서명 OTP를 위해 서버에 LIQUOR_SIGNATURE_OTP_PEPPER(16자 이상) 환경 변수가 필요합니다. Railway 등 배포 환경 변수를 확인해 주세요.',
    }
  }
  if (
    msg.includes('LIQUOR_SIGNATURE_TARGET_PHONE_ENCRYPTION_KEY') ||
    ...
    return {
      ...
      message:
        '전화번호 저장을 위해 서버에 LIQUOR_SIGNATURE_TARGET_PHONE_ENCRYPTION_KEY(64자 hex 또는 아무 문자열)가 필요합니다. Railway 환경 변수를 확인해 주세요.',
    }
  }
 ...
 *   requireLiquorIndustrySignatureUser: import('express').RequestHandler,
 ...
 * }} ctx
 */
export function registerLiquorSignatureUserApi(apiRouter, ctx) {
  const {
    pool,
    requireAuth,
  ...
  apiRouter.get('/liquor/signatures/templates', ...chain, async (req, res) => {
    try {
      const userGa = parseGaId(req.user?.gaId)
      if (userGa == null) {
      ...
      const r = await pool.query(
        ...
        SELECT
          ...
          COALESCE(
            (SELECT COUNT(*)::int FROM liquor_pdf_template_fields f WHERE f.template_id = t.pdf_template_id),
            0
          ) AS pdf_field_count,
          COALESCE(
            (SELECT COUNT(*)::int FROM liquor_pdf_template_fields f
      ...
      if (pdfIds.length > 0) {
        const fr = await pool.query(
          ...
          FROM liquor_pdf_template_fields
      ...
      res.json({
        ...
        templates: r.rows.map((row) => {
          ...
          const senderList = listSenderFieldsForLiquorSignatureTemplate(pdfRows, sm)
          return {
            id: row.id,
            title: row.title,
  ...
  apiRouter.get('/liquor/signatures/templates/:templateId/confirmation-fields', ...chain, async (req, res) => {
    try {
      const userGa = parseGaId(req.user?.gaId)
      if (userGa == null) {
        res.status(400).json({ ok: false, message: 'GA 컨텍스트가 없습니다.' })
      ...
      const { row, error, status } = await assertLiquorSignatureTemplateAccess(pool, templateId, userGa, false)
      if (error) {
      ...
      if (modeErr) {
        res
          ...
          .json({ ok: false, code: 'liquor_signature_template_not_confirmation_only', message: modeErr.error })
        return
      }
      const r = await pool.query(
        `
  ...
  apiRouter.get('/liquor/signatures/customers/search', ...chain, async (req, res) => {
    try {
      const userGa = parseGaId(req.user?.gaId)
      const uid = getAuthUserId(req)
      if (!uid) {
  ...
  apiRouter.post('/liquor/signatures/send-sessions', ...chain, async (req, res) => {
    const client = await pool.connect()
    /** @type {Record<string, unknown>} */
    const debugCtx = {
      route: 'liquor/signatures/send-sessions',
    ...
    try {
      ...
      const senderMaps = senderValuesByLiquorSignatureTemplates(senderRoot, parsed.ids)
      ...
      let liquorSignatureTemplatesOrdered = []
      ...
      for (const tid of parsed.ids) {
        const tacc = await assertLiquorSignatureTemplateAccess(client, tid, userGa, false)
        if (tacc.error) {
      ...
      logLiquorSignatureSendSessionFailure(debugCtx, e)
  ...
  apiRouter.post(
    '/liquor/signatures/send-sessions/attachment-upload',
    ...chain,
    (req, res, next) => {
    ...
    async (req, res) => {
      try {
        ...
        const storageKey = buildLiquorSignatureObjectKey(`send-attachments/${uid}/${randomUUID()}/${displayBase}`)
        try {
  ...
  apiRouter.get('/liquor/signatures/send-sessions', ...chain, async (req, res) => {
    try {
      const userGa = parseGaId(req.user?.gaId)
      const uid = getAuthUserId(req)
      if (!uid) {
      ...
      if (qSearch) {
        ...
        searchClause = `
          AND (
            ...
            OR EXISTS (
              SELECT 1 FROM liquor_signature_document_instances cdi2
      ...
      const dataSql = `
        ...
        FROM liquor_signature_send_sessions s
        INNER JOIN customers c ON c.id = s.customer_id
        LEFT JOIN liquor_signature_identity_sessions ivs ON ivs.id = s.identity_session_id
        LEFT JOIN LATERAL (
          SELECT
  ...
  apiRouter.patch('/liquor/signatures/send-sessions/:id/cancel', ...chain, async (req, res) => {
    const client = await pool.connect()
    try {
      const userGa = parseGaId(req.user?.gaId)
      const uid = getAuthUserId(req)
      ...
      const docCk = await client.query(
        `
        SELECT EXISTS (
          SELECT 1 FROM liquor_signature_document_instances
  ...
  apiRouter.get('/liquor/signatures/send-sessions/:id', ...chain, async (req, res) => {
    try {
      const userGa = parseGaId(req.user?.gaId)
      const uid = getAuthUserId(req)
      if (!uid) {
  ...
  apiRouter.get(
    '/liquor/signatures/send-sessions/:sendSessionId/documents/:documentInstanceId/signed-pdf',
    ...chain,
    async (req, res) => {
      try {
        const userGa = parseGaId(req.user?.gaId)
        ...
        const d = await pool.query(
          ...
          SELECT
            ...
            c.name AS customer_name
          FROM liquor_signature_document_instances cdi
          INNER JOIN liquor_signature_send_sessions s2 ON s2.id = cdi.send_session_id
  ...
  apiRouter.get('/liquor/signatures/send-sessions/:sendSessionId/evidence.pdf', ...chain, async (req, res) => {
    try {
      const userGa = parseGaId(req.user?.gaId)
      const uid = getAuthUserId(req)
      if (!uid) {
