...
import { getLiquorSignatureOtpPepper, isRunningInProduction } from '../lib/liquorSignatureOtpConfig.js'
import { encryptContractTargetPhoneDigits } from '../lib/liquorSignatureStoredPhone.js'
import {
  assertSenderFieldValuesFilled,
  insertSenderPrefillDocumentValues,
  senderValuesByLiquorSignatureTemplates,
} from '../services/liquorSignatureSenderPrefill.js'
import {
  assertLiquorSignatureFieldSettingsValidForActivate,
  insertFixedPrefillDocumentValues,
  normalizeLiquorSignatureFieldInputRole,
  seedLiquorSignatureTemplateFieldSettings,
  effectiveLiquorSignatureFieldRole,
} from '../services/liquorSignatureTemplateFieldSettings.js'
import {
  insertConfirmationItemsForSendSession,
  listConfirmationItemsWithValues,
  parseConfirmationItemsFromBody,
} from '../services/liquorSignatureConfirmationItems.js'
import { listFields } from '../pdf-engine/repository/liquorPdfTemplateRepo.js'

const LST_PREFIX = 'lst_'
const LSTF_PREFIX = 'lstf_'
const LSTCF_PREFIX = 'lstcf_'
const LSPKG_PREFIX = 'lspkg_'
const LSS_PREFIX = 'lss_'
const LSDI_PREFIX = 'lsdi_'
 ...
 */
export function assertLiquorConfirmationOnlyTemplateRow(row) {
...
async function allocateUniqueConfirmationFieldKey(client, templateId, baseKey) {
  ...
  for (let n = 0; n < 200; n += 1) {
    const ck = await client.query(
      `SELECT 1 FROM liquor_signature_template_confirmation_fields WHERE template_id = $1 AND field_key = $2 LIMIT 1`,
      [templateId, k],
    )
    if (ck.rowCount === 0) {
      return k
...
async function nextConfirmationFieldSortOrder(client, templateId) {
  const r = await client.query(
    `SELECT COALESCE(MAX(sort_order), -1)::int AS m FROM liquor_signature_template_confirmation_fields WHERE template_id = $1`,
    [templateId],
  )
...
function hashPhoneDigits(digits) {
  const pepper = getLiquorSignatureOtpPepper()
...
async function countPdfEngineFields(client, pdfTemplateId) {
  const r = await client.query(
    `SELECT COUNT(*)::int AS c FROM liquor_pdf_template_fields WHERE template_id = $1`,
    [pdfTemplateId],
  )
  return Number(r.rows[0]?.c ?? 0)
}

export async function assertLiquorSignatureTemplateAccess(client, templateId, effectiveGaId, isSuper) {
  const r = await client.query(
    `SELECT * FROM liquor_signature_templates WHERE id = $1 LIMIT 1`,
    [templateId],
  )
  const row = r.rows[0]
  if (!row) {
...
async function assertPackageAccess(client, packageId, effectiveGaId, isSuper) {
  const r = await client.query(`SELECT * FROM liquor_signature_packages WHERE id = $1 LIMIT 1`, [packageId])
  const row = r.rows[0]
  if (!row) {
...
export function buildTargetPhoneSnapshot(digits) {
  ...
  if (isRunningInProduction() && !encrypted) {
    throw new Error('[contract phone] LIQUOR_SIGNATURE_TARGET_PHONE_ENCRYPTION_KEY is not set')
  }
  return {
...
export async function generateUniqueLinkCode(client) {
  for (let i = 0; i < 8; i += 1) {
    ...
    const ck = await client.query(
      `SELECT 1 FROM liquor_signature_send_sessions WHERE link_code = $1 LIMIT 1`,
      [code],
    )
    if (ck.rowCount === 0) {
      return code
...
export function parseTemplateIdsArray(raw) {
  ...
  for (const x of raw) {
    ...
    if (!id.startsWith(LST_PREFIX) || id.length < LST_PREFIX.length + 4) {
 ...
 *   attachPlatformContext: import('express').RequestHandler,
 *   forbidInsurerManagerApi: import('express').RequestHandler,
 *   requireLiquorIndustrySignatureAdmin: import('express').RequestHandler,
 *   attachLiquorSignatureContext: import('express').RequestHandler,
 *   handleDbError: (e: unknown, req: import('express').Request, res: import('express').Response) => void,
 * }} ctx
 */
export function registerLiquorSignatureTemplateApi(apiRouter, ctx) {
  const {
    pool,
    requireAuth,
  ...
  apiRouter.get('/liquor/signature-templates/templates', ...chain, async (req, res) => {
    try {
      ...
      const r = await pool.query(
        ...
        FROM liquor_signature_templates t
        LEFT JOIN liquor_pdf_templates p ON p.id = t.pdf_template_id
        LEFT JOIN (
          SELECT template_id, COUNT(*)::int AS document_instance_count
          FROM liquor_signature_document_instances
          GROUP BY template_id
        ) di ON di.template_id = t.id
        LEFT JOIN (
          SELECT template_id, COUNT(*)::int AS package_item_count
          FROM liquor_signature_package_items
          GROUP BY template_id
  ...
  apiRouter.get('/liquor/signature-templates/templates/:id', ...chain, async (req, res) => {
    try {
      const effectiveGa = await resolveEffectiveGaId(pool, req)
      const isSuper = isSuperAdminRole(req.user?.role)
      const { row, error, status } = await assertLiquorSignatureTemplateAccess(
        pool,
        req.params.id,
        effectiveGa,
        isSuper,
      ...
      const fieldsR = await pool.query(
        `SELECT COUNT(*)::int AS c FROM liquor_signature_template_fields WHERE template_id = $1`,
        [row.id],
      )
      const pdfR = row.pdf_template_id
        ? await pool.query(
            `SELECT id, title, storage_key, page_count, is_active FROM liquor_pdf_templates WHERE id = $1 LIMIT 1`,
            [row.pdf_template_id],
          )
        : { rows: [] }
      ...
      if (row.pdf_template_id != null) {
        ...
        const stR = await pool.query(
          `SELECT field_key, input_role, fixed_value FROM liquor_signature_template_field_settings WHERE template_id = $1`,
          [row.id],
        )
        const sm = new Map()
        ...
        for (const pf of pdfFields) {
          ...
          const st = sm.get(fk)
          const role = effectiveLiquorSignatureFieldRole(pf, st)
      ...
      res.json({
        ...
        template: {
          ...
          pageCount: row.page_count,
          gaId: row.ga_id,
          liquorSignatureTemplateFieldsCount: fieldsR.rows[0]?.c ?? 0,
          fieldInputSettings,
          pdfEngine: pr
            ? {
                id: pr.id,
  ...
  apiRouter.post('/liquor/signature-templates/templates', ...chain, async (req, res) => {
    ...
    try {
      ...
      await client.query(
        `
        INSERT INTO liquor_signature_templates (
      ...
      if (pdfTemplateId != null && Number.isInteger(pdfTemplateId)) {
        await seedLiquorSignatureTemplateFieldSettings(client, id, pdfTemplateId)
      }
      res.status(201).json({ ok: true, success: true, data: { id } })
    } catch (e) {
      if (e instanceof Error && e.message.includes('LIQUOR_SIGNATURE_OTP_PEPPER')) {
        res.status(500).json({ ok: false, message: '서버 설정 오류(LIQUOR_SIGNATURE_OTP_PEPPER)입니다.' })
        return
      }
      handleDbError(e, req, res)
    } finally {
  ...
  apiRouter.patch('/liquor/signature-templates/templates/:id', ...chain, async (req, res) => {
    const client = await pool.connect()
    try {
      const effectiveGa = await resolveEffectiveGaId(pool, req)
      const isSuper = isSuperAdminRole(req.user?.role)
      const acc = await assertLiquorSignatureTemplateAccess(client, req.params.id, effectiveGa, isSuper)
      if (acc.error) {
      ...
      params.push(row.id)
      await client.query(
        `UPDATE liquor_signature_templates SET ${sets.join(', ')}, updated_at = NOW() WHERE id = $${params.length}`,
        params,
      )
      if (pdfSettingsResync === 'clear') {
        await client.query(`DELETE FROM liquor_signature_template_field_settings WHERE template_id = $1`, [row.id])
      } else if (pdfSettingsResync != null && typeof pdfSettingsResync === 'number') {
        await client.query(`DELETE FROM liquor_signature_template_field_settings WHERE template_id = $1`, [row.id])
        await seedLiquorSignatureTemplateFieldSettings(client, row.id, pdfSettingsResync)
      }
  ...
  apiRouter.patch('/liquor/signature-templates/templates/:id/status', ...chain, async (req, res) => {
    const client = await pool.connect()
    try {
      const effectiveGa = await resolveEffectiveGaId(pool, req)
      const isSuper = isSuperAdminRole(req.user?.role)
      ...
      if (status === 'active') {
        ...
        if (mode === 'confirmation_only') {
          const cntR = await client.query(
            `SELECT COUNT(*)::int AS c FROM liquor_signature_template_confirmation_fields WHERE template_id = $1`,
            [row.id],
          )
          ...
          const settingsOk = await assertLiquorSignatureFieldSettingsValidForActivate(client, row.id, Number(pid))
          if (!settingsOk.ok) {
      ...
      await client.query(`UPDATE liquor_signature_templates SET status = $2, updated_at = NOW() WHERE id = $1`, [
        row.id,
        status,
      ])
      res.json({ ok: true, success: true })
  ...
  apiRouter.post('/liquor/signature-templates/templates/:id/duplicate', ...chain, async (req, res) => {
    const client = await pool.connect()
    try {
      ...
      const fieldsR = await client.query(`SELECT * FROM liquor_signature_template_fields WHERE template_id = $1`, [
        src.id,
      ])
      for (const f of fieldsR.rows) {
        const nf = newId(LSTF_PREFIX)
        await client.query(
          `
          INSERT INTO liquor_signature_template_fields (
      ...
      if (settingsDup.rowCount === 0 && src.pdf_template_id != null) {
        await seedLiquorSignatureTemplateFieldSettings(client, newTid, Number(src.pdf_template_id))
      }
      const confDup = await client.query(
        `
        SELECT field_key, label, input_type, required, sort_order, placeholder, help_text
        FROM liquor_signature_template_confirmation_fields
        WHERE template_id = $1
      ...
      for (const cf of confDup.rows) {
        const ncf = newId(LSTCF_PREFIX)
        await client.query(
          `
          INSERT INTO liquor_signature_template_confirmation_fields (
  ...
  apiRouter.patch('/liquor/signature-templates/templates/:id/field-input-settings', ...chain, async (req, res) => {
    const client = await pool.connect()
    try {
      const effectiveGa = await resolveEffectiveGaId(pool, req)
      const isSuper = isSuperAdminRole(req.user?.role)
      ...
      for (const it of items) {
        ...
        let role = normalizeLiquorSignatureFieldInputRole(it?.inputRole ?? it?.input_role)
  ...
  apiRouter.get('/liquor/signature-templates/templates/:id/confirmation-fields', ...chain, async (req, res) => {
    try {
      const effectiveGa = await resolveEffectiveGaId(pool, req)
      const isSuper = isSuperAdminRole(req.user?.role)
      const { row, error, status } = await assertLiquorSignatureTemplateAccess(
      ...
      const modeErr = assertLiquorConfirmationOnlyTemplateRow(row)
      if (modeErr) {
  ...
  apiRouter.post('/liquor/signature-templates/templates/:id/confirmation-fields', ...chain, async (req, res) => {
    const client = await pool.connect()
    try {
      const effectiveGa = await resolveEffectiveGaId(pool, req)
      const isSuper = isSuperAdminRole(req.user?.role)
      ...
      const tpl = acc.row
      const modeErr = assertLiquorConfirmationOnlyTemplateRow(tpl)
      if (modeErr) {
        res.status(modeErr.status).json({ ok: false, message: modeErr.error })
        return
      }
      ...
      const ins = await client.query(
        `SELECT * FROM liquor_signature_template_confirmation_fields WHERE id = $1 LIMIT 1`,
        [id],
      )
      const fr = ins.rows[0]
  ...
  apiRouter.put('/liquor/signature-templates/templates/:id/confirmation-fields/:fieldId', ...chain, async (req, res) => {
    const client = await pool.connect()
    try {
      if (
      ...
      if (!fieldId.startsWith(LSTCF_PREFIX)) {
        res.status(400).json({ ok: false, message: 'fieldId가 올바르지 않습니다.' })
        return
      }
      const cur = await client.query(
        `SELECT * FROM liquor_signature_template_confirmation_fields WHERE id = $1 AND template_id = $2 LIMIT 1`,
        [fieldId, tpl.id],
      )
      if (cur.rowCount === 0) {
  ...
  apiRouter.delete('/liquor/signature-templates/templates/:id/confirmation-fields/:fieldId', ...chain, async (req, res) => {
    const client = await pool.connect()
    try {
      const effectiveGa = await resolveEffectiveGaId(pool, req)
      const isSuper = isSuperAdminRole(req.user?.role)
      ...
      const del = await client.query(
        `DELETE FROM liquor_signature_template_confirmation_fields WHERE id = $1 AND template_id = $2`,
        [fieldId, tpl.id],
      )
      if (del.rowCount === 0) {
        res.status(404).json({ ok: false, message: '확인 항목을 찾을 수 없습니다.' })
  ...
  apiRouter.delete('/liquor/signature-templates/templates/:id', ...chain, async (req, res) => {
    const client = await pool.connect()
    try {
      const effectiveGa = await resolveEffectiveGaId(pool, req)
      const isSuper = isSuperAdminRole(req.user?.role)
      ...
      const row = acc.row
      const cntR = await client.query(
        `SELECT COUNT(*)::int AS c FROM liquor_signature_document_instances WHERE template_id = $1`,
        [row.id],
      )
      ...
      const pkgR = await client.query(
        `SELECT COUNT(*)::int AS c FROM liquor_signature_package_items WHERE template_id = $1`,
        [row.id],
      )
      ...
      await client.query(`DELETE FROM liquor_signature_templates WHERE id = $1`, [row.id])
      res.json({ ok: true, success: true })
    } catch (e) {
      handleDbError(e, req, res)
    } finally {
  ...
  apiRouter.get('/liquor/signature-templates/packages', ...chain, async (req, res) => {
    try {
      const effectiveGa = await resolveEffectiveGaId(pool, req)
      const isSuper = isSuperAdminRole(req.user?.role)
      const params = /** @type {unknown[]} */ ([])
      ...
      const r = await pool.query(
        `SELECT * FROM liquor_signature_packages ${where} ORDER BY updated_at DESC LIMIT 300`,
        params,
      )
  ...
  apiRouter.get('/liquor/signature-templates/packages/:id', ...chain, async (req, res) => {
    try {
      const effectiveGa = await resolveEffectiveGaId(pool, req)
      const isSuper = isSuperAdminRole(req.user?.role)
      ...
      const items = await pool.query(
        ...
        FROM liquor_signature_package_items i
        JOIN liquor_signature_templates t ON t.id = i.template_id
  ...
  apiRouter.post('/liquor/signature-templates/packages', ...chain, async (req, res) => {
    const client = await pool.connect()
    try {
      const effectiveGa = await resolveEffectiveGaId(pool, req)
      if (effectiveGa == null) {
      ...
      const isSuper = isSuperAdminRole(req.user?.role)
      await client.query('BEGIN')
      const pkgId = newId(LSPKG_PREFIX)
      const uid = getAuthUserId(req)
      const description = req.body?.description != null ? String(req.body.description) : null
      ...
      await client.query(
        `
        INSERT INTO liquor_signature_packages (id, title, description, status, ga_id, created_by_user_id, created_at, updated_at)
      ...
      for (let idx = 0; idx < itemsRaw.length; idx += 1) {
        ...
        if (!tid.startsWith(LST_PREFIX)) {
          await client.query('ROLLBACK')
          res.status(400).json({ ok: false, message: 'items[].templateId가 올바르지 않습니다.' })
          return
        }
        const tacc = await assertLiquorSignatureTemplateAccess(client, tid, effectiveGa, isSuper)
        if (tacc.error) {
          await client.query('ROLLBACK')
        ...
        await client.query(
          `
          INSERT INTO liquor_signature_package_items (id, package_id, template_id, sort_order, required, created_at)
  ...
  apiRouter.patch('/liquor/signature-templates/packages/:id', ...chain, async (req, res) => {
    const client = await pool.connect()
    try {
      const effectiveGa = await resolveEffectiveGaId(pool, req)
      const isSuper = isSuperAdminRole(req.user?.role)
      ...
      if (Array.isArray(req.body?.items)) {
        ...
        await client.query(`DELETE FROM liquor_signature_package_items WHERE package_id = $1`, [row.id])
      ...
      if (sets.length > 0) {
        params.push(row.id)
        await client.query(
          `UPDATE liquor_signature_packages SET ${sets.join(', ')}, updated_at = NOW() WHERE id = $${params.length}`,
          params,
        )
      }
  ...
  apiRouter.patch('/liquor/signature-templates/packages/:id/status', ...chain, async (req, res) => {
    const client = await pool.connect()
    try {
      const effectiveGa = await resolveEffectiveGaId(pool, req)
      const isSuper = isSuperAdminRole(req.user?.role)
      ...
      await client.query(`UPDATE liquor_signature_packages SET status = $2, updated_at = NOW() WHERE id = $1`, [row.id, status])
      res.json({ ok: true, success: true })
    } catch (e) {
      handleDbError(e, req, res)
    } finally {
  ...
  apiRouter.post('/liquor/signature-templates/send-sessions', ...chain, async (req, res) => {
    const client = await pool.connect()
    try {
      const effectiveGa = await resolveEffectiveGaId(pool, req)
      const isSuper = isSuperAdminRole(req.user?.role)
      ...
      let liquorSignatureTemplatesOrdered = /** @type {{ id: string, title: string, version: number, required: number, pdfHash: string | null, pdfTemplateId: number | null }[]} */ ([])

      await client.query('BEGIN')

      if (hasPkg) {
        const pid = String(pPkg).trim()
        if (!pid.startsWith(LSPKG_PREFIX)) {
          await client.query('ROLLBACK')
        ...
        liquorSignatureTemplatesOrdered = items.rows.map((x) => ({
          id: x.id,
          title: x.title,
          version: x.version,
      ...
      if (hasTpl) {
        ...
        liquorSignatureTemplatesOrdered = []
        for (const tid of parsed.ids) {
          ...
          liquorSignatureTemplatesOrdered.push({
            id: t.id,
            title: t.title,
            version: t.version,
            required: 1,
      ...
      const senderMaps = senderValuesByLiquorSignatureTemplates(
        senderRoot,
        liquorSignatureTemplatesOrdered.map((x) => String(x.id)),
      )
      ...
      for (const ct of liquorSignatureTemplatesOrdered) {
      ...
      await client.query(
        `
        INSERT INTO liquor_signature_send_sessions (
      ...
      for (let i = 0; i < liquorSignatureTemplatesOrdered.length; i += 1) {
        const ct = liquorSignatureTemplatesOrdered[i]
        const docId = newId(LSDI_PREFIX)
        await client.query(
          `
          INSERT INTO liquor_signature_document_instances (
      ...
      res.status(201).json({
        ...
        sendSession: {
          ...
          documentCount: liquorSignatureTemplatesOrdered.length,
  ...
  apiRouter.get('/liquor/signature-templates/send-sessions', ...chain, async (req, res) => {
    try {
      const effectiveGa = await resolveEffectiveGaId(pool, req)
      const isSuper = isSuperAdminRole(req.user?.role)
      const params = /** @type {unknown[]} */ ([])
      let where = `
        FROM liquor_signature_send_sessions s
  ...
  apiRouter.get('/liquor/signature-templates/send-sessions/:id', ...chain, async (req, res) => {
    try {
      const effectiveGa = await resolveEffectiveGaId(pool, req)
      const isSuper = isSuperAdminRole(req.user?.role)
      ...
      if (docIds.length > 0) {
        const evRows = await pool.query(
          ...
          SELECT DISTINCT ON (document_instance_id)
            ...
            signed_pdf_file_id,
            signed_pdf_hash
          FROM liquor_signature_evidences
          WHERE send_session_id = $1
