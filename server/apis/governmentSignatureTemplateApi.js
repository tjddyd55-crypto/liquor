...
import {
  assertGovSignatureFieldSettingsValidForActivate,
  insertFixedPrefillDocumentValues,
  normalizeGovSignatureFieldInputRole,
  seedGovernmentSignatureTemplateFieldSettings,
...
const GST_PREFIX = 'gst_'
const GSTF_PREFIX = 'gstf_'
const GSTCF_PREFIX = 'gstcf_'
const GSPKG_PREFIX = 'gspkg_'
 ...
 */
export function assertGovConfirmationOnlyTemplateRow(row) {
...
async function allocateUniqueConfirmationFieldKey(client, templateId, baseKey) {
  ...
  for (let n = 0; n < 200; n += 1) {
    const ck = await client.query(
      `SELECT 1 FROM gov_signature_template_confirmation_fields WHERE template_id = $1 AND field_key = $2 LIMIT 1`,
      [templateId, k],
    )
    if (ck.rowCount === 0) {
      return k
...
async function nextConfirmationFieldSortOrder(client, templateId) {
  const r = await client.query(
    `SELECT COALESCE(MAX(sort_order), -1)::int AS m FROM gov_signature_template_confirmation_fields WHERE template_id = $1`,
    [templateId],
  )
...
export function hashPhoneDigitsForGovSignature(digits) {
  const pepper = getGovernmentSignatureOtpPepper()
  return createHash('sha256').update(`${digits}|${pepper}`, 'utf8').digest('hex')
}

export async function assertProfileForGovSignatureSend(client, profileId, ownerUserId) {
...
export function buildTargetPhoneSnapshot(digits) {
  ...
  return {
    ...
    target_phone_hash: hashPhoneDigitsForGovSignature(digits),
    target_phone_masked: maskKrMobileForDisplay(digits),
  }
}

export async function generateUniqueSignToken(client) {
  for (let i = 0; i < 8; i += 1) {
    const code = randomBytes(32).toString('base64url')
    const ck = await client.query(
      `SELECT 1 FROM gov_signature_send_sessions WHERE sign_token = $1 LIMIT 1`,
      [code],
    )
    if (ck.rowCount === 0) {
      return code
  ...
  throw new Error('sign_token_collision')
}
...
export function parseTemplateIdsArray(raw) {
  ...
  for (const x of raw) {
    ...
    if (!id.startsWith(GST_PREFIX) || id.length < GST_PREFIX.length + 4) {
 ...
 * }} ctx
 */
export function registerGovernmentSignatureTemplateApi(apiRouter, ctx) {
  const {
    pool,
    requireAuth,
    ...
    try {
      ...
      const r = await pool.query(
        ...
        LEFT JOIN (
          ...
          FROM gov_signature_package_items
          GROUP BY template_id
  ...
  apiRouter.get('/government-support/signature-templates/:id', ...chain, async (req, res) => {
    try {
      ...
      const fieldsR = await pool.query(
        `SELECT COUNT(*)::int AS c FROM gov_signature_template_fields WHERE template_id = $1`,
        [row.id],
      )
      ...
      if (row.pdf_template_id != null) {
        ...
        const stR = await pool.query(
          `SELECT field_key, input_role, fixed_value FROM gov_signature_template_field_settings WHERE template_id = $1`,
          [row.id],
        )
        const sm = new Map()
        ...
        for (const pf of pdfFields) {
          ...
          const st = sm.get(fk)
          const role = effectiveGovSignatureFieldRole(pf, st)
      ...
      res.json({
        ...
        template: {
          ...
          pageCount: row.page_count,
          ownerUserId: row.owner_user_id,
          governmentSignatureTemplateFieldsCount: fieldsR.rows[0]?.c ?? 0,
          fieldInputSettings,
          pdfEngine: pr
            ? {
                id: pr.id,
  ...
  apiRouter.post('/government-support/signature-templates', ...chain, async (req, res) => {
    ...
    try {
      ...
      if (pdfTemplateId != null && Number.isInteger(pdfTemplateId)) {
        await seedGovernmentSignatureTemplateFieldSettings(client, id, pdfTemplateId)
      }
      res.status(201).json({ ok: true, success: true, data: { id } })
    } catch (e) {
      if (e instanceof Error && e.message.includes('GOV_SIGNATURE_OTP_PEPPER')) {
        res.status(500).json({ ok: false, message: '서버 설정 오류(GOV_SIGNATURE_OTP_PEPPER)입니다.' })
        return
      }
      handleDbError(e, req, res)
    } finally {
  ...
  apiRouter.patch('/government-support/signature-templates/:id', ...chain, async (req, res) => {
    ...
    try {
      ...
      params.push(row.id)
      await client.query(
        `UPDATE gov_signature_templates SET ${sets.join(', ')}, updated_at = NOW() WHERE id = $${params.length}`,
        params,
      )
      if (pdfSettingsResync === 'clear') {
        await client.query(`DELETE FROM gov_signature_template_field_settings WHERE template_id = $1`, [row.id])
      } else if (pdfSettingsResync != null && typeof pdfSettingsResync === 'number') {
        await client.query(`DELETE FROM gov_signature_template_field_settings WHERE template_id = $1`, [row.id])
        await seedGovernmentSignatureTemplateFieldSettings(client, row.id, pdfSettingsResync)
      }
  ...
  apiRouter.patch('/government-support/signature-templates/:id/status', ...chain, async (req, res) => {
    ...
    try {
      ...
      if (status === 'active') {
        ...
        if (mode === 'confirmation_only') {
          const cntR = await client.query(
            `SELECT COUNT(*)::int AS c FROM gov_signature_template_confirmation_fields WHERE template_id = $1`,
            [row.id],
          )
          ...
          const settingsOk = await assertGovSignatureFieldSettingsValidForActivate(client, row.id, Number(pid))
          if (!settingsOk.ok) {
      ...
      await client.query(`UPDATE gov_signature_templates SET status = $2, updated_at = NOW() WHERE id = $1`, [
        row.id,
        status,
      ])
      res.json({ ok: true, success: true })
  ...
  apiRouter.post('/government-support/signature-templates/:id/duplicate', ...chain, async (req, res) => {
    const client = await pool.connect()
    try {
      ...
      for (const f of fieldsR.rows) {
        const nf = newId(GSTF_PREFIX)
        await client.query(
          `
          INSERT INTO gov_signature_template_fields (
      ...
      for (const s of settingsDup.rows) {
        await client.query(
          `
          INSERT INTO gov_signature_template_field_settings (template_id, field_key, input_role, fixed_value, created_at, updated_at)
      ...
      if (settingsDup.rowCount === 0 && src.pdf_template_id != null) {
        await seedGovernmentSignatureTemplateFieldSettings(client, newTid, Number(src.pdf_template_id))
      }
      ...
      for (const cf of confDup.rows) {
        const ncf = newId(GSTCF_PREFIX)
        await client.query(
          `
          INSERT INTO gov_signature_template_confirmation_fields (
  ...
  apiRouter.patch('/government-support/signature-templates/:id/field-input-settings', ...chain, async (req, res) => {
    ...
    try {
      ...
      for (const it of items) {
        ...
        let role = normalizeGovSignatureFieldInputRole(it?.inputRole ?? it?.input_role)
  ...
  apiRouter.post('/government-support/signature-templates/:id/confirmation-fields', ...chain, async (req, res) => {
    ...
    try {
      ...
      const tpl = acc.row
      const modeErr = assertGovConfirmationOnlyTemplateRow(tpl)
      if (modeErr) {
        res.status(modeErr.status).json({ ok: false, message: modeErr.error })
        return
      }
      ...
      const ins = await client.query(
        `SELECT * FROM gov_signature_template_confirmation_fields WHERE id = $1 LIMIT 1`,
        [id],
      )
      const fr = ins.rows[0]
  ...
  apiRouter.put('/government-support/signature-templates/:id/confirmation-fields/:fieldId', ...chain, async (req, res) => {
    const client = await pool.connect()
    try {
      if (
      ...
      if (!fieldId.startsWith(GSTCF_PREFIX)) {
        res.status(400).json({ ok: false, message: 'fieldId가 올바르지 않습니다.' })
        return
      }
      const cur = await client.query(
        `SELECT * FROM gov_signature_template_confirmation_fields WHERE id = $1 AND template_id = $2 LIMIT 1`,
        [fieldId, tpl.id],
      )
      if (cur.rowCount === 0) {
      ...
      await client.query(
        `
        UPDATE gov_signature_template_confirmation_fields
  ...
  apiRouter.delete('/government-support/signature-templates/:id/confirmation-fields/:fieldId', ...chain, async (req, res) => {
    ...
    try {
      ...
      const del = await client.query(
        `DELETE FROM gov_signature_template_confirmation_fields WHERE id = $1 AND template_id = $2`,
        [fieldId, tpl.id],
      )
      if (del.rowCount === 0) {
        res.status(404).json({ ok: false, message: '확인 항목을 찾을 수 없습니다.' })
  ...
  apiRouter.delete('/government-support/signature-templates/:id', ...chain, async (req, res) => {
    ...
    try {
      ...
      const row = acc.row
      const cntR = await client.query(
        `SELECT COUNT(*)::int AS c FROM gov_signature_document_instances WHERE template_id = $1`,
        [row.id],
      )
      ...
      const pkgR = await client.query(
        `SELECT COUNT(*)::int AS c FROM gov_signature_package_items WHERE template_id = $1`,
        [row.id],
      )
      ...
      await client.query(`DELETE FROM gov_signature_templates WHERE id = $1`, [row.id])
      res.json({ ok: true, success: true })
    } catch (e) {
      handleDbError(e, req, res)
    } finally {
  ...
  apiRouter.get('/government-support/signature-packages', ...chain, async (req, res) => {
    try {
      const ownerUserId = await resolveGovSignatureOwnerUserId(pool, req)
      const isSuper = false
      const params = /** @type {unknown[]} */ ([])
      ...
      const r = await pool.query(
        `SELECT * FROM gov_signature_packages ${where} ORDER BY updated_at DESC LIMIT 300`,
        params,
      )
  ...
  apiRouter.get('/government-support/signature-packages/:id', ...chain, async (req, res) => {
    try {
      const ownerUserId = await resolveGovSignatureOwnerUserId(pool, req)
      const isSuper = false
      const acc = await assertPackageAccess(pool, req.params.id, ownerUserId, isSuper)
      if (acc.error) {
        res.status(acc.status ?? 400).json({ ok: false, message: acc.error })
        return
      }
      ...
      const items = await pool.query(
        ...
        FROM gov_signature_package_items i
        JOIN gov_signature_templates t ON t.id = i.template_id
  ...
  apiRouter.post('/government-support/signature-packages', ...chain, async (req, res) => {
    const client = await pool.connect()
    try {
      const ownerUserId = await resolveGovSignatureOwnerUserId(pool, req)
      if (ownerUserId == null) {
      ...
      const isSuper = false
      await client.query('BEGIN')
      const pkgId = newId(GSPKG_PREFIX)
      const uid = getAuthUserId(req)
      const description = req.body?.description != null ? String(req.body.description) : null
      ...
      await client.query(
        `
        INSERT INTO gov_signature_packages (id, title, description, status, owner_user_id, created_by_user_id, created_at, updated_at)
        VALUES ($1, $2, $3, $4, $5, $6, NOW(), NOW())
        `,
        [pkgId, title, description, st, ownerUserId, uid || null],
      )
      ...
      for (let idx = 0; idx < itemsRaw.length; idx += 1) {
        ...
        await client.query(
          `
          INSERT INTO gov_signature_package_items (id, package_id, template_id, sort_order, required, created_at)
  ...
  apiRouter.patch('/government-support/signature-packages/:id', ...chain, async (req, res) => {
    const client = await pool.connect()
    try {
      const ownerUserId = await resolveGovSignatureOwnerUserId(pool, req)
      const isSuper = false
      const acc = await assertPackageAccess(client, req.params.id, ownerUserId, isSuper)
      if (acc.error) {
        res.status(acc.status ?? 400).json({ ok: false, message: acc.error })
        return
      }
      ...
      if (Array.isArray(req.body?.items)) {
        ...
        await client.query(`DELETE FROM gov_signature_package_items WHERE package_id = $1`, [row.id])
      ...
      if (sets.length > 0) {
        params.push(row.id)
        await client.query(
          `UPDATE gov_signature_packages SET ${sets.join(', ')}, updated_at = NOW() WHERE id = $${params.length}`,
          params,
        )
      }
  ...
  apiRouter.patch('/government-support/signature-packages/:id/status', ...chain, async (req, res) => {
    const client = await pool.connect()
    try {
      const ownerUserId = await resolveGovSignatureOwnerUserId(pool, req)
      const isSuper = false
      ...
      await client.query(`UPDATE gov_signature_packages SET status = $2, updated_at = NOW() WHERE id = $1`, [row.id, status])
      res.json({ ok: true, success: true })
    } catch (e) {
      handleDbError(e, req, res)
    } finally {
